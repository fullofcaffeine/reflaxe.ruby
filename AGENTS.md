...
- Gradual adoption exception: existing Rails/Poc `.erb` files may be tracked only as Rails-owned external source in explicit interop/adoption fixtures or user apps. Haxe must consume those files through typed contracts such as `Template.external("path") : Template<TLocals>` and typed extern/facade wrappers for Ruby helpers/services. Do not rewrite mixed-app fixtures to fake HHX ownership; the point is to prove Haxe can wrap and gradually replace real Rails code safely.
- For quick Rails PoCs, support this workflow: prototype in pure Rails/Ruby/ERB, wrap stable Ruby services/components with typed Haxe externs and `Template.external`, then migrate pieces to Haxe/HHX over time without breaking Ruby callers. Existing Ruby should consume generated Haxe constants and generated partials exactly as normal hand-written Rails artifacts.
...
- RailsHx template paths should move toward typed template references. `Template.named("...")`, `Template.external("...")`, and `@:railsTemplate("...")` strings must remain literal-only and should be validated against RailsHx-owned template classes/generated artifacts where possible; external Rails/Poc templates should be explicitly marked with `Template.external` and checked for safe path shape at minimum.
