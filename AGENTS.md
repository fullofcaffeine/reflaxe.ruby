...
- For ActionView work, prefer the typed `rails.action_view.H` helper facade over direct `HtmlNode`/`HtmlAttr` constructors for new compiler-owned template slices. Use `H.partial((Template.named(...) : Template<TLocals>), locals)` for typed partial composition. All of these should lower through `@:railsTemplateAst(...)`; raw ERB must remain explicit with `@:railsAllowRawErb` and should be treated as a migration bridge, not the destination.
