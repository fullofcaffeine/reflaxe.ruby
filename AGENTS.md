...
- HHX helper tags that take labels, such as `<link_to>`, `<field_label>`, and `<submit>`, may use `text="..."`, static child text, or `${...}` expression children. Keep nested markup/component slots as explicit future work unless the parser/lowerer has typed coverage and tests.
