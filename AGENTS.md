...
- RailsHx jobs should stay ActiveJob-native: `@:railsJob` classes emit `ActiveJob::Base`, queue/retry/discard metadata should lower to Rails class macros, and typed enqueue helpers should be generated from `perform(...)` rather than maintained as stringly/manual wrappers.
...
- RailsHx routing is Rails-owned in phase 1: `config/routes.rb` is the source of truth and RailsHx generates Haxe externs from `rails routes`. Do not implement Haxe-owned route emission until `docs/railshx-routing-design.md` parity gates exist in tests; Rails remains the route-helper naming oracle even if Haxe later emits `config/routes.rb`.
