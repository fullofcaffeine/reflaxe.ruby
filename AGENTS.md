...
- Hard rule for ActionView: app-facing Rails templates must be authored as typed Rails HHX inline markup (`return <div>...</div>`) through `@:railsTemplateAst(...)`. Raw ERB strings (`public static var body:String = '<% ... %>'`, `erb/template` string fields, or copied `.erb` source) are not acceptable for new examples or default paths. ERB is an output format produced by the compiler, not the authoring format.
