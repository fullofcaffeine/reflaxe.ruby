    ...
    class Mailer
      ...
      def self.parse(argv)
        ...
        parser = OptionParser.new do |opts|
          ...
          opts.on("--test-adapter ADAPTER", "minitest, rspec, or auto") { |value| options[:test_adapter] = value }
      ...
      def render_test
        [
          ...
          "",
          "// Generated by HXRuby::Generators::Mailer.",
          "// Demonstrates: a Haxe-authored #{TestAdapter.framework_name(@test_adapter)} source file for the mailer.",
          "// The compiler emits an ordinary test artifact under #{TestAdapter.output_root(@test_adapter)}/**.",
          TestAdapter.metadata_line(@test_adapter),
          "@:railsTest(\"#{test_artifact_path}\")",
          "class #{test_class} extends ModelTestCase {",
          "\t@:railsTests",
          "\tstatic function define():Void {",
          "\t\ttest(\"typed parameterized mailer builds a message\", () -> {",
          "\t\t\tvar mail = #{@mailer_name}.withParams({",
      ...
      def test_class
        "#{@mailer_name}#{TestAdapter.class_suffix(@test_adapter)}"
      end

      def test_artifact_path
        "mailers/#{Common.file_name(@mailer_name)}_haxe_#{TestAdapter.file_suffix(@test_adapter)}"
      end
