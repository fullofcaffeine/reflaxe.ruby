  ...
  module Generators
    class Adopt
      ...
      def render_service(package, haxe_class, native_name, contract = nil)
        if contract && contract.fetch(:source_kind, "ruby_source") == "rbs"
          return render_rbs_service(package, haxe_class, native_name, contract)
        end
      ...
      # RBS-backed adoption uses the RubyHx-wide renderer in source-order mode
      # so extracting the reusable pipeline cannot reorder an established app
      # contract or change its generated snapshot.
      def render_rbs_service(package, haxe_class, native_name, contract)
        HXRuby::Rbs::HaxeExternRenderer.new(
          contract: contract,
      ...
      def rbs_contracts
        @rbs_contracts ||= @rbs_sources.flat_map do |source|
          ...
          begin
            HXRuby::Rbs::SourceParser.new(real_source, relative_output_path(source_path)).contracts
          rescue HXRuby::Rbs::Error => error
            raise Error, error.message
          end
        end
      ...
      def render_devise_auth_contract(scope)
        ...
        [
          ...
          *route_meta,
          ...
          "",
          "\t@:railsFilterMethod(#{Common.haxe_string("authenticate_#{scope.fetch(:scope)}!")})",
          ...
          "\t}",
          "",
          "\t@:railsRequiresFilter(#{Common.haxe_string("authenticate_#{scope.fetch(:scope)}!")})",
      ...
      def render_devise_sessions_new_view(scope)
        ...
        [
          ...
          "\t\t\t\t</form_with>",
          "\t\t\t\t<link_to url=${AuthLinks.signUpPath(#{auth_class}.scope)} class=\"devisehx-secondary-link\">Create an account</link_to>",
          "\t\t\t</section>",
          "\t\t</main>;",
          "\t}",
          "}",
      ...
      def render_devise_registrations_new_view(scope)
        ...
        [
          ...
          "\t\t\t\t</form_with>",
          "\t\t\t\t<link_to url=${AuthLinks.signInPath(#{auth_class}.scope)} class=\"devisehx-secondary-link\">Already have an account?</link_to>",
          "\t\t\t</section>",
          "\t\t</main>;",
          "\t}",
          "}",
      ...
      def render_devise_passwords_new_view(scope)
        ...
        [
          ...
          "\t\t\t\t</form_with>",
          "\t\t\t\t<link_to url=${AuthLinks.signInPath(#{auth_class}.scope)} class=\"devisehx-secondary-link\">Back to sign in</link_to>",
          "\t\t\t</section>",
          "\t\t</main>;",
          "\t}",
          "}",
