    ...
    class App
      ...
      def render_gem_layers_doc
        [
          ...
          "   ```",
          "",
          "   The optional `--yard` command is service-specific: the constant must be explicit, the checked Ruby file stays inside the app root, and only complete positional `@param`/`@return` signatures with supported scalar, nilable, Boolean, or Array types are generated. Generic `--gem ... --discover` / `--write contracts` also finds those tags automatically under the Bundler-resolved gem `lib` root. It canonicalizes every source, rejects symlink escapes, and makes a YARD-backed constant precise-or-omitted across reopened files. Unsupported methods remain review-marked and omitted instead of becoming `Dynamic`; Ruby source is never executed.",
          "",
