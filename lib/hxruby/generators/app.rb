    ...
    class App
      ...
      def render_agents
        [
          ...
          "- Target-shaped Haxe APIs are valid when intentional and useful for migration, interop, Rails predictability, or familiar framework mental models. They should still be typed/toolable and should leverage Haxe's types, macros, generated refs, properties, completion, and compile-time diagnostics where practical.",
          ...
          "- Prefer intent-first RailsHx APIs over mechanical Ruby-object mirrors in app code. Low-level typed stores/facades may exist, but controllers should use semantic/property-style surfaces such as `this.flash.notice(\"Saved\")`, `this.flash.alert(\"Admins only\")`, `request.format().json()`, and `response.status()` when available, while RailsHx still emits ordinary Rails output. Target-shaped Haxe is valid when intentional and useful at an interop boundary, not as accidental syntax copying.",
          ...
          "- For realtime DOM updates, prefer the normal Hotwire shape: render current state on page load, subscribe with typed HHX such as `<turbo_stream_from stream=${...} />`, and broadcast server-rendered typed partials through `TurboStreams.broadcast*To(...)`. Use Haxe JS `MyChannel.client.subscribe(...)` only for custom non-DOM protocols or genuinely client-owned behavior; reserve `Consumer.subscribeExternal(...)` for Rails-owned channels without a Haxe contract.",
      ...
      def render_gem_layers_doc
        [
          ...
          "   ```",
          "",
          "   The optional `--rbs` and `--yard` commands are service-specific: the constant must be explicit, the canonical input must stay inside the app root, and only complete positional signatures with supported scalar, nilable, Symbol, or Array types are generated. Missing files, symlink escapes, malformed RBS structure, and unsupported method shapes fail or remain precise review omissions; formal metadata never becomes a broad fallback type. Generic `--gem ... --discover` / `--write contracts` also finds YARD tags automatically under the Bundler-resolved gem `lib` root, canonicalizes every source, rejects symlink escapes, and makes a YARD-backed constant precise-or-omitted across reopened files. Ruby source is never executed.",
          "",
          "   Capture what RailsHx can prove mechanically: gem version/path, public constants, RBS signatures, automatically discovered gem YARD signatures, explicitly selected app-service YARD signatures, source-defined modules/methods, Rails routes, generated migrations, initializers, model concerns, controller helpers, and test helpers.",
          "",
          ...
          "",
          "   For Devise, start from the upstream GPT 5.5 Pro design packet before implementing a reusable companion layer:",
          "   https://github.com/fullofcaffeine/reflaxe.ruby/blob/main/docs/railshx-devisehx-gpt55-prompt.md",
          "",
