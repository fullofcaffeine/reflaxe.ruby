    ...
    class App
      ...
      def render_agents
        [
          ...
          "- Prefer intent-first RailsHx APIs over mechanical Ruby-object mirrors in app code. Low-level typed stores/facades may exist, but controllers should use semantic surfaces such as `this.flash.notice(\"Saved\")` or `this.flash.alert(\"Admins only\")` when available, while RailsHx still emits ordinary Rails output.",
