...
module HXRuby
  module Generators
    class Controller
      ...
      def self.run(argv, hotwire: false)
        new(parse(argv).merge(hotwire: hotwire)).run
      end
      ...
      def validate_static_options!
        ...
        if @with_hotwire && (!@model_name || !@with_templates || !@actions.include?("index") || !@actions.include?("create"))
          raise Error, "--hotwire requires --model, --templates, and both index and create actions"
        end
      end
      ...
      def render_controller
        ...
        imports << "import shared.#{hotwire_contract_class};" if @with_hotwire
      ...
      def render_action(action)
        body = if @model_name && action == "index" && @with_templates
                 ...
                 [
                   ...
                   ("\t\tif (#{@resource_name}.persisted()) #{hotwire_contract_class}.broadcastCreated(#{@resource_name});" if @with_hotwire),
                   redirect_line,
                 ].compact
      ...
      def render_view(action)
        ...
        [
          ...
          ("import shared.#{hotwire_contract_class};" if @with_hotwire && action == "index"),
          ("import shared.#{hotwire_hooks_class};" if @with_hotwire && action == "index"),
          "",
          "// Generated by HXRuby::Generators::Controller.",
          ...
          ("@:railsDomTargets(#{hotwire_hooks_class}.targetId())" if @with_hotwire && action == "index"),
      ...
      def render_model_index_view_body
        if @with_hotwire
          return [
            "\t\treturn <main class=\"railshx-generated-view\">",
            "\t\t\t<h1>${locals.title}</h1>",
            "\t\t\t<p>This realtime scaffold renders and broadcasts one typed HHX row partial.</p>",
            "\t\t\t<turbo_stream_from stream=${#{hotwire_contract_class}.streamName()} />",
            "\t\t\t<ul id=${#{hotwire_hooks_class}.targetId()}>",
            "\t\t\t\t<for \${#{@resource_name} in locals.#{@table_name}}>",
            "\t\t\t\t\t<partial template=${#{hotwire_contract_class}.rowTemplate()} locals=${#{hotwire_contract_class}.rowLocals(#{@resource_name})} />",
            "\t\t\t\t</for>",
            "\t\t\t</ul>",
            "\t\t</main>;",
          ]
