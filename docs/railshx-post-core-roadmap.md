...
- Runtime lane uses Rails test disk storage, ActiveStorage tables, real blobs,
  signed-ID attachment, direct-upload blob metadata, direct-upload file field
  rendering, download/read assertions, and purge assertions.
...
- Direct-upload helpers, blob metadata facades, and common attachable hash
  builders are implemented with typed APIs; variants/previews, validations,
  analyzers, and richer builder coverage remain deferred behind tracked beads.
