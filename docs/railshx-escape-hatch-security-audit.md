...
| SQL/string-bearing query APIs | Typed refs/builders such as `Todo.f.title`, criteria objects, `Order.many`, `Projection.pluck`, `Group.count`. | Future raw SQL/string APIs must be explicit and auditable; current policy forbids casual addition. | [RailsHx SQL And String-Bearing API Policy](railshx-sql-string-policy.md) classifies typed defaults, checked literals, and explicit escapes; `test:sql-string-policy` rejects raw query strings in canonical examples. | `docs/railshx-sql-string-policy.md`; `npm run test:sql-string-policy`. | `haxe.ruby-bjv.4.1`, `haxe.ruby-bjv.10`. |
