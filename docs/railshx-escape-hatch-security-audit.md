...
| DeviseHx companion escapes | Generated `DeviseScope<T>`, typed Devise modules, typed sanitizer field refs, typed route/helper metadata, HHX auth helpers, and Rails-owned Devise runtime. | `DeviseModule.unsafeCustom(...)`, `DeviseParams.unsafePermit(...)`, `WardenAccess.unsafeWarden(...)`, `DeviseRoutes.uncheckedRuby(...)`, and explicit Rails-owned ERB/force boundaries for adopted Devise views. | Known Devise modules and required schema columns are compile/generator checked; unsafe sanitizer keys must be string literals; Warden and route escapes are named unsafe/unchecked; Devise route adoption fails closed for dynamic options; canonical todoapp uses typed scopes/helpers and keeps Devise runtime in Rails/Bundler. | `npm run test:devisehx-core`; `npm run test:devisehx-controller`; `npm run test:rails-adopt-generator`; `npm run test:routes-generator`; `npm run test:todoapp-rails`; DeviseHx release lane docs. | `haxe.ruby-bjv.6.4.10`, `haxe.ruby-bjv.4.2`, `haxe.ruby-bjv.4.4`. |
