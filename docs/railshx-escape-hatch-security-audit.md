...
| `haxe_ruby-08b2` | P1 | Generator output containment | Reject manifest-owned, forced, manifest-file, route-extern, and parent-directory symlink escapes before any write or clean mutation. |
...
| Compiler-erased companion Ruby expressions | Typed companion macros validate domain metadata and expose ordinary typed Haxe calls; generated app contracts stay free of raw authority. | Internal `@:rubyExtensionExpr` carriers on a type marked with both `@:rubyNoEmit` and `@:rubyAllowRaw`. | Schema 1 accepts only a literal template or constant-string template argument, validates keys/indexes/placeholders, and substitutes values through normal typed Ruby/HHX lowering. Core contains no companion package/schema/helper/tag coupling; `test:devisehx-core` guards that boundary without banning ordinary Rails `devise_for`, and a non-Devise compile fixture proves the carrier is reusable. | `npm run test:devisehx-core`; `npm run test:devisehx-controller`; `docs/compiler-metadata.md`. | Owned by `haxe_ruby-8q9`; do not add a registry or runtime plugin path without a concrete second lifecycle requirement. |
...
| Generator/adoption source files | Deterministic metadata from Rails source, RBS, YARD, routes, schemas, Bundler, or typed Haxe metadata. | Suggest-only discovery, review-marked source-shape skeletons, and future LLM-assisted patches. | Missing explicit source/RBS/YARD files fail closed; explicit RBS is canonicalized inside the canonical app root, rejects symlink escapes and malformed structure, and emits only precise-or-omitted methods without broad fallback types; source-backed adoption parses without executing app or gem code; generic gem adoption resolves its root through Bundler, canonicalizes `lib` sources, rejects symlink escapes, and makes any YARD-backed constant precise-or-omitted across reopenings; packages, Ruby constants, locals, Haxe type strings, and template paths are validated before Haxe source is emitted; generators protect existing wrappers. | `npm run test:rails-adopt-generator`; `npm run test:yard-adopt-generator`; gradual adoption, gem-layer, and Ruby extension docs. | `haxe.ruby-bjv.4.4`, `haxe.ruby-bjv.6`. |
| Generator output paths | App-relative regular files canonically contained by the declared output root. | `--force` may take over a colliding regular file, but is not a path-containment escape. | The shared writer canonicalizes existing ancestors, rejects symlink output leaves, uses no-follow writes, protects the ownership manifest, and validates every cleanup path before deletion. The route extern atomic writer uses the same boundary. | `npm run test:generator-common`; generated artifact ownership docs. | `haxe_ruby-08b2`. |
| DeviseHx companion escapes | Generated `DeviseScope<T>`, typed Devise modules, typed sanitizer field refs, typed route/helper metadata, ordinary HHX Rails helpers, and Rails-owned Devise runtime. | `DeviseModule.unsafeCustom(...)`, `DeviseParams.unsafePermit(...)`, `WardenAccess.unsafeWarden(...)`, and explicit Rails-owned route/ERB/force boundaries for shapes DeviseHx cannot author. | Known Devise modules and required schema columns are compile/generator checked; unsafe sanitizer keys must be literals; Warden access is loudly named; unsupported or dynamic route adoption stays Rails-owned instead of entering a nonexistent raw-route API; canonical todoapp keeps Devise runtime in Rails/Bundler. | `npm run test:devisehx-core`; `npm run test:devisehx-controller`; `npm run test:rails-adopt-generator`; `npm run test:routes-generator`; `npm run test:todoapp-rails`; DeviseHx release lane docs. | `haxe.ruby-bjv.6.4.10`, `haxe.ruby-bjv.4.2`, `haxe.ruby-bjv.4.4`. |
| `Dynamic` in app-facing APIs | Typed facades, generated externs, abstracts, and typed payload/locals/contracts. Generated route helpers use `RouteParam` so required segments are no longer advertised as `Dynamic`; relation-level ActiveRecord `find` uses typed scalar ID overloads instead of `Dynamic`; controller direct render/redirect status kwargs use `Status` through `RenderOptions`/`RedirectOptions`; request negotiation uses typed `RequestFormat` values for `format()`, `formats()`, and `contentMimeType()`; Turbo app code uses `EventTarget` targets and helpers such as `Turbo.addFetchRequestHeader(...)` instead of touching runtime fetch options directly; ActionCable client subscriptions require a typed `Consumer` handle and typed `Action<TData>` perform tokens; ActionMailer mail options use typed recipient/layout wrappers and typed attachment helpers; ActiveJob `performNow(...)` preserves the `perform(...)` return type and `performLater(...)` returns a typed job handle; ActiveSupport subscriptions use opaque handles without implicit `Dynamic` construction; ActiveStorage default `attach(...)` uses typed signed-id/blob and `Attachable`/`Attachables` values. | `Dynamic` only at real runtime interop boundaries, explicit Haxe escapes such as `attachUnchecked(...)`, `Attachable.unchecked(...)`, `Attachables.unchecked(...)`, `performUnchecked(...)`, `MailAddress.unchecked(...)`, `MailLayout.unchecked(...)`, `AttachmentValue.unchecked(...)`, or `attachments().addUnchecked(...)`, Rails model/object route params via `RouteParam.model(...)`, Rails-owned ActiveStorage blob metadata/direct-upload headers, broad Rails-owned render payloads such as `json`/`locals`, or generated review placeholders. | Some lower-level surfaces still use runtime-owned shapes: JS/Turbo fetch/response internals, controller render payload objects, ActiveStorage metadata/header hashes, generator-inferred unknown types, and explicit unchecked interop calls. Haxe's explicit `Dynamic` can still bypass ordinary static checks, so canonical APIs must not expose it casually. | Compatibility matrix, controller/Turbo/generator docs, `npm run test:todoapp-rails`, `npm run test:turbo`, `npm run test:routes-generator`, `npm run test:active-record-model`, `npm run test:action-cable`, `npm run test:action-mailer`, `npm run test:active-job`, `npm run test:instrumentation`, `npm run test:active-storage`. | `haxe.ruby-bjv.4.2`. |
