...
| Generator/adoption source files | Deterministic metadata from Rails source, RBS, YARD, routes, schemas, Bundler, or typed Haxe metadata. | Suggest-only discovery, review-marked source-shape skeletons, and future LLM-assisted patches. | Missing explicit source/RBS/YARD files fail closed; source-backed adoption uses `Ripper`/RBS parsing without executing app or gem code; explicit inputs stay inside the app/output root; generic gem adoption resolves its root through Bundler, canonicalizes `lib` sources, rejects symlink escapes, and makes any YARD-backed constant precise-or-omitted across reopenings; packages, Ruby constants, locals, Haxe type strings, and template paths are validated before Haxe source is emitted; generators protect existing wrappers. | `npm run test:rails-adopt-generator`; `npm run test:yard-adopt-generator`; gradual adoption, gem-layer, and Ruby extension docs. | `haxe.ruby-bjv.4.4`, `haxe.ruby-bjv.6`. |
