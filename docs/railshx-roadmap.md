...
- Minimal ActiveRecord model surface through `rails.active_record.Base<T>` and `rails.macros.ModelMacro`, including generated model-owned field refs such as `Todo.titleField : Field<Todo, String>` and params/form scopes such as `Todo.railsParamKey : ModelKey<Todo>`.
- Minimal ActionController/strong params surface through `rails.action_controller.Base`, `Params`, and `ParamsMacro`, including typed model-scope validation for `ParamsMacro.requirePermit(this.params(), Todo.railsParamKey, [Todo.titleField])`.
