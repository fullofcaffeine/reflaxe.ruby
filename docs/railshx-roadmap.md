...
- Initial typed ActiveRecord relation chain through `Relation<T, TCriteria>`, typed model/relation `where({...})` criteria, typed association `includes(...)`/`joins(...)`, typed `find(...)`/`findBy({...})`, `order(Todo.f.title.asc())`, `limit(...)`, `first()`, and `toArray()`, while emitting normal Rails `ActiveRecord::Relation` calls.
