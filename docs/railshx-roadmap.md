...
- `haxe.ruby-wpi.3`: implement typed `Relation<T, TCriteria>` and ActiveRecord query DSL. The first slices exist for typed model/relation `where`/`rewhere` criteria that survive inferred `all`/`distinct`/`none`/`reverseOrder`/`readOnly`/`or`/`merge`/`select`/`order`/`reorder`/`limit`/`offset` relation chains, typed expression predicates through `Expr`/`Predicate`, typed association `includes`/`joins`, typed Rails scope declarations through `@:railsScope` and `@:railsDefaultScope`, typed `find`/`findBy`/`exists`, typed `count`, typed field/expression order tokens including `Order.many([...])`, typed single-field `pluck`, typed named multi-field `Projection.pluck`, grouped aggregate `Projection.group`, typed `Group.count` and `Group.countHaving` for supported map key types, typed aggregate predicates through `Aggregate`, typed `minimum`/`maximum`, typed `sum`/`average` for `Int` field refs, and `first`/`last`/`toArray`; joined aggregate projections, multi-model projections, named criteria artifacts, explicit SQL escape APIs, and broader grouped-query key support remain.
