...
- Initial typed ActiveRecord relation chain through `Relation<T, TCriteria>`, typed model/relation `where({...})` criteria, typed association `includes(...)`/`joins(...)`, typed `find(...)`/`findBy({...})`/`exists({...})`, `all()`, `distinct()`, `order(Todo.f.title.asc())`, `limit(...)`, `offset(...)`, `count()`, `first()`, `last()`, and `toArray()`, while emitting normal Rails `ActiveRecord::Relation` calls.
