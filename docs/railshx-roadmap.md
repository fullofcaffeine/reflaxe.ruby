...
- `haxe.ruby-wpi.3`: implement typed `Relation<T, TCriteria>` and ActiveRecord query DSL. The first slices exist for typed model/relation `where` criteria that survive inferred `all`/`distinct`/`order`/`limit`/`offset` relation chains, typed association `includes`/`joins`, typed static scope methods returning relations, typed `find`/`findBy`/`exists`, typed `count`, typed field order tokens, typed single-field `pluck`, and `first`/`last`/`toArray`; richer scope DSLs, aggregations, multi-field projections, association-aware nested queries, named criteria artifacts, and richer docs remain.
