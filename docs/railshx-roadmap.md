...
- `haxe.ruby-wpi.3`: implement typed `Relation<T, TCriteria>` and ActiveRecord query DSL. The first slices exist for typed model/relation `where`/`rewhere` criteria that survive inferred `all`/`distinct`/`none`/`reverseOrder`/`readOnly`/`or`/`merge`/`select`/`order`/`reorder`/`limit`/`offset` relation chains, typed expression predicates through `Expr`/`Predicate`, typed association `includes`/`joins`, typed Rails scope declarations through `@:railsScope` and `@:railsDefaultScope`, typed `find`/`findBy`/`exists`, typed `count`, typed field/expression order tokens including `Order.many([...])`, typed single-field `pluck`, typed named multi-field `Projection.pluck`, typed `Group.count` and `Group.countHaving` for supported map key types, typed aggregate predicates through `Aggregate`, typed `minimum`/`maximum`, typed `sum`/`average` for `Int` field refs, and `first`/`last`/`toArray`; selected aggregate result objects, multi-model projections, named criteria artifacts, explicit SQL escape APIs, and broader grouped-query key support remain.
...
For the current query authoring contract, see [RailsHx Typed ActiveRecord Query Guide](railshx-query-guide.md). For the expression/Arel boundary, see [RailsHx Query Expression Design](railshx-query-expression-design.md). For the design rationale behind named multi-field projections and grouped counts, see [RailsHx Multi-Field Projection And Grouping Design](railshx-projections-grouping-design.md). The short version: keep chains Rails-shaped, use generated field/association refs for compile-time checks, use `Expr`/`Predicate`/`Aggregate` for typed SQL functions and predicates, use `Projection.pluck(...)`/`Group.count(...)`/`Group.countHaving(...)` when result types depend on field refs, and cross from `Relation<T>` to loaded arrays with `toArray()` at controller/template boundaries.
