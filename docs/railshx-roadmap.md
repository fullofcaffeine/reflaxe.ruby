...
- `haxe.ruby-wpi.3`: implement typed `Relation<T, TCriteria>` and ActiveRecord query DSL. The first slices exist for typed model/relation `where`/`rewhere` criteria that survive inferred `all`/`distinct`/`none`/`reverseOrder`/`readOnly`/`or`/`merge`/`select`/`order`/`reorder`/`limit`/`offset` relation chains, typed association `includes`/`joins`, typed static scope methods returning relations, typed `find`/`findBy`/`exists`, typed `count`, typed field order tokens including `Order.many([...])`, typed single-field `pluck`, typed named multi-field `Projection.pluck`, typed `Group.count` for supported map key types, typed `minimum`/`maximum`, typed `sum`/`average` for `Int` field refs, and `first`/`last`/`toArray`; richer scope DSLs, richer aggregations, multi-model projections, association-aware nested queries, named criteria artifacts, and broader grouped-query key support remain.
