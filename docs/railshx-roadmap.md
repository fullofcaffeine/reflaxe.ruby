...
- Initial typed ActiveRecord relation chain through `Relation<T, TCriteria>`, typed model/relation `where({...})` and `rewhere({...})` criteria, typed association `includes(...)`/`joins(...)`, typed `find(...)`/`findBy({...})`/`exists({...})`, `all()`, `distinct()`, `select(Todo.f.title)`, `order(Todo.f.title.asc())`, `reorder(Todo.f.id.desc())`, `limit(...)`, `offset(...)`, typed single-field `pluck(Todo.f.title)`, typed `minimum(Todo.f.id)` / `maximum(Todo.f.id)`, `count()`, `first()`, `last()`, and `toArray()`, while emitting normal Rails `ActiveRecord::Relation` calls.
...
- `haxe.ruby-wpi.3`: implement typed `Relation<T, TCriteria>` and ActiveRecord query DSL. The first slices exist for typed model/relation `where`/`rewhere` criteria that survive inferred `all`/`distinct`/`select`/`order`/`reorder`/`limit`/`offset` relation chains, typed association `includes`/`joins`, typed static scope methods returning relations, typed `find`/`findBy`/`exists`, typed `count`, typed field order tokens, typed single-field `pluck`, typed `minimum`/`maximum`, and `first`/`last`/`toArray`; richer scope DSLs, aggregations, multi-field projections, association-aware nested queries, named criteria artifacts, and richer docs remain.
...
For the current query authoring contract, see [RailsHx Typed ActiveRecord Query Guide](railshx-query-guide.md). For the selected v1 design for named multi-field projections and grouped counts, see [RailsHx Multi-Field Projection And Grouping Design](railshx-projections-grouping-design.md). The short version: keep chains Rails-shaped, use generated field/association refs for compile-time checks, and cross from `Relation<T>` to loaded arrays with `toArray()` at controller/template boundaries.
