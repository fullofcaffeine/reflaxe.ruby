...
Typed Rails templates are part of the RailsHx destination. `haxe.ruby-wpi.12` now has an initial typed partial/component path through Rails HHX plus `H`/`HtmlNode`/`HtmlAttr`, including typed conditionals, loops, partial composition, route-helper links, form helper lowering, and helper-tag child text. The remaining work is fuller typed field projection, component ergonomics, expression slots, and migration of larger raw ERB strings without losing Rails-native output.
