...
The implemented boundary is deliberately small. Generated/companion macros use
`@:rubyExtensionExpr` for checked expressions, `@:railsFilterMethod` and
`@:railsRequiresFilter` for lifecycle flow, `@:railsClassMacro` for validated
model class macros, `@:railsTestInclude` for test modules, and
`RouteDecl.extension(...)` for an opaque validated route line/manifest entry.
Compiler core contains no DeviseHx package/schema/helper/tag coupling, and
`test:devisehx-core` enforces that source boundary while deliberately allowing
ordinary Rails syntax such as `devise_for`. There is no runtime registry,
plugin loader, or package discovery protocol; those would be justified only by
...
final class UserAuth {
	// DeviseHx owns this metadata. Its macros read the literal Rails facts and
	// hand only generic checked Rails/Ruby contracts to compiler core.
	@:deviseHxRoute({
		schema: 1,
...
Low-level `Auth.signIn(...)` accepts only the checked Devise 4.9/5.0 overlap
`{force: Bool, store: Bool}` and emits those Ruby keywords. Bypass behavior uses
the explicit `Auth.bypassSignIn(...)` helper; Devise 4.9 deprecated the old
`bypass` option and Devise 5 removed it. Unknown, dynamic, or stored option bags
...
The DeviseHx macro reads the app-local metadata, validates it, and hands a
generic checked expression plus test include to compiler core. Generated output
uses Rails-native `sign_in(:user, user)` / `sign_out(:user)` without evaluating
...
class DeviseLoginView {
	public static function render(resource:User):HtmlNode {
		return <main>
			...
			<link_to url=${AuthLinks.signUpPath(UserAuth.scope)}>Sign up</link_to>
			<link_to url=${AuthLinks.editRegistrationPath(UserAuth.scope)}>
				Account settings
			</link_to>
...
`AuthLinks` is not a runtime widget. Its DeviseHx macro requires a direct
generated scope field such as `UserAuth.scope`, reads companion-owned
`@:deviseHxRoute` metadata, and passes a checked expression to the compiler's
generic extension carrier. The result is ordinary Rails helpers:
`user_session_path`, `new_user_session_path`,
...
function-returned scopes fail closed because the companion cannot safely prove
which Devise mapping they describe. Templates compose `AuthLinks.*Path(...)`
with ordinary typed `<link_to>` and `<button_to>` HHX tags; this keeps the core
HHX vocabulary Rails-shaped and the sign-out flow Turbo-safe.
...
- `currentRequired(...)` is allowed when the action is protected by the matching
  ...
  boundary.
- `-D railshx_strict_required_filters` enables compile-time flow checks
...
- `beforeAction(UserAuth.authenticate)` consumes generated generic
  `@:railsFilterMethod` metadata. Plain strings are not accepted.
...
The adoption inventory records the Devise modules and scope facts separately;
there is no `@:deviseExtern` authoring API. The typed contract must not cause
RailsHx to regenerate the Rails-owned model.
...
The DeviseHx macro accepts only direct generated scope fields carrying its
`@:deviseHxRoute(...)` metadata, validates the Devise vocabulary, then passes
an opaque generic route extension to core and emits ordinary Rails:
...
| Dynamic routes | Rails-owned route + generated route externs | No raw DeviseHx route escape; keep ownership in Rails |
