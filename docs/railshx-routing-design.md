...
# Source: src_haxe/routes/AppRoutes.hx
...
- Add the public DSL markers and macros under `std/rails/**`.
- Implement route ref/literal validation before adding broad resources coverage.
- Extract route IR and emit generated `config/routes.rb` plus a route manifest.
- Add Haxe-owned route sync/parity tasks on top of the existing Rails-owned
  route helper generator.
- Add route snapshots, negative compile tests, Rails request tests, and
  ownership tests before switching greenfield generators/templates to
  Haxe-owned routes.
