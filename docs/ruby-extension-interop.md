...
Filesystem-backed macros/generators must fail closed. If an API references `app/models`, `app/views`, `sig`, `rbs_collection`, a gem path, or any other file/directory source, missing or unsafe paths must be compile/generator errors by default. Generator inputs that become Haxe source, such as package names, Ruby constants, locals, and type strings, must be validated before files are written. Provide an explicit unchecked escape hatch only when there is a real synthetic/test/adoption reason, and name it accordingly (`external`, `unchecked`, or similar).
