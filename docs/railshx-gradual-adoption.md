...
field-name collisions fail closed. Decimal precision/scale from `db/schema.rb`
is preserved as checked `@:railsColumn` metadata. Ambiguous associations are
emitted as review comments instead of guessed `belongsTo` metadata, and
unsupported DB types fail unless `--allow-dynamic` is explicit.
