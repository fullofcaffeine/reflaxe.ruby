...
| Projections/grouping/aggregates | Single-field `pluck`, named `Projection.pluck`, `Group.count`, `Group.countHaving`, typed aggregate predicates, and min/max/sum/average initial slices exist. | Multi-model projections, richer grouped keys, selected aggregate aliases/result objects, and broader SQL function builders need design/implementation. | `haxe.ruby-bjv.3.1`, `haxe.ruby-bjv.3.7` |
...
| ActionMailer | Initial typed mailer class/template slice exists. | Parameterized mailers, previews, attachments, multipart defaults, delivery/test helpers, runtime delivery assertions, and integration with jobs need production coverage. | `haxe.ruby-bjv.3.5`, `haxe.ruby-if3` |
| ActiveJob | Typed job/enqueue slice exists, including contextual lifecycle lowering and a generated Rails `ActiveJob::TestHelper` runtime lane for queue/enqueue/perform behavior. | Adapter-specific behavior beyond the test adapter, serialization contracts, retry/discard edge cases, richer queue naming policy, and failure/retry runtime assertions need hardening. | `haxe.ruby-bjv.3.5`, `haxe.ruby-71o` |
| ActiveStorage | Initial typed attachment refs exist. | Attachment validations, variants/previews, blobs, direct uploads, purge/analyze helpers, service config assumptions, and runtime storage tests need production coverage. | `haxe.ruby-bjv.3.5`, `haxe.ruby-cfj` |
...
| ActionCable | Typed channel/subscription slice exists, including params, streams, broadcast payloads, typed JS consumers/actions, and generated Rails `ActionCable::Channel::TestCase` runtime coverage for subscribe/stream/perform/broadcast. | Connection identifiers, stream stop/unsubscribe lifecycle, reject/confirm auth seams, and broader client callback/runtime behavior need hardening. | `haxe.ruby-bjv.3.6`, `haxe.ruby-tke` |
