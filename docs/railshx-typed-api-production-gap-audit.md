...
| HHX/ActionView | HHX templates, typed locals, partials, components, route/helper calls, forms, typed form select options, typed form field errors, layout helpers, slots, and raw ERB escape are implemented as initial slices. | Collection rendering, Rails helper breadth, remaining form builder options, accessibility helper breadth, `content_for`/layout edge cases, component ergonomics, and richer external-template validation need production coverage. | `haxe.ruby-bjv.3.4` |
