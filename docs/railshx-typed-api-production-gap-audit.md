...
| ActionMailer | Initial typed mailer class/template slice exists, including generated Rails runtime coverage for multipart HTML/text body rendering, headers, string attachments, and `deliver_now` through the Rails test delivery collection. | Parameterized mailers, previews, richer attachment hashes, mailer/job integration, and generated preview/test-helper UX need production design and coverage. | `haxe.ruby-bjv.3.5`, `haxe.ruby-if3` |
| ActiveJob | Typed job/enqueue slice exists, including contextual lifecycle lowering and a generated Rails `ActiveJob::TestHelper` runtime lane for queue/enqueue/perform, serialization/deserialization, and retry re-enqueue behavior. | Adapter-specific behavior beyond the test adapter, discard edge cases, richer queue naming policy, and broader failure diagnostics need hardening. | `haxe.ruby-bjv.3.5`, `haxe.ruby-71o` |
