...
| ActiveStorage | Initial typed attachment refs exist, including generated Rails runtime coverage for ActiveStorage tables, test disk service, blob signed-ID attach, typed `io`/`filename`/`content_type` hash attachables, download/read, and purge for one and many attachments. | Attachment validations, variants/previews, blob metadata facades, direct-upload helper generation, analyzer hooks, and richer attachable builders need production design and coverage. | `haxe.ruby-bjv.3.5`, `haxe.ruby-cfj`, `haxe.ruby-9sl` |
