...
| ActionMailer | Initial typed mailer class/template slice exists, including generated Rails runtime coverage for multipart HTML/text body rendering, headers, string attachments, and `deliver_now` through the Rails test delivery collection. Typed parameterized `.with(...)` call sites and `params[:key]` reads now have static/runtime coverage. | Generated parameterized wrapper macros, previews, richer attachment hashes, mailer/job integration, and generated preview/test-helper UX need implementation and runtime coverage. | `haxe.ruby-bjv.3.5`, `haxe.ruby-if3`, `haxe.ruby-z34.1`, `haxe.ruby-z34.2`, `haxe.ruby-z34.3`, `haxe.ruby-4tx` |
