...
| Model metadata | Initial columns, associations, validations including presence/uniqueness, enums, callbacks, and schema metadata exist. | Additional validation options, custom validators, dependent/inverse/through variants, polymorphic/STI decisions, nested attributes, scopes from associations, and callback coverage need audit-driven expansion. | `haxe.ruby-bjv.3.2` |
