...
| ActiveStorage | Initial typed attachment refs exist, including generated Rails runtime coverage for ActiveStorage tables, test disk service, blob signed-ID attach, download/read, and purge for one and many attachments. | Attachment validations, variants/previews, blob metadata facades, direct-upload helper generation, analyzer hooks, and typed builders for common attachable hash shapes need production design and coverage. | `haxe.ruby-bjv.3.5`, `haxe.ruby-cfj` |
