...
| Controllers/params | Typed strong params, request/response facades including `RequestFormat`, filters, status tokens, `respond_to` format collector, flash/session/cookies, and template rendering exist. | Richer content negotiation, variants, rescue_from, around/after filters, auth hooks, streaming/send_file, CSRF policy, route/controller validation, and request specs need coverage. | `haxe.ruby-bjv.3.3` |
