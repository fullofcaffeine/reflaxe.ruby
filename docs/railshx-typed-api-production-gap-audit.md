...
| Controllers/params | Typed strong params, request/response facades including `RequestFormat`, contextual `lifecycle` declarations for filters and `rescue_from`, status tokens, `respond_to` format collector, typed `send_file`/`send_data` options, flash/session/cookies, and template rendering exist. | Richer content negotiation, variants, auth hooks, streaming responses beyond file/data sends, CSRF policy, route/controller validation, and request specs need coverage. | `haxe.ruby-bjv.3.3` |
