...
| Migrations | `@:railsMigration(...)` plus `MigrationOperation` values | Prefer macro/generator inference from models where safe, with explicit operations for irreversible or external DB changes. | Medium; DB behavior needs strong fail-closed checks. |
| HHX templates/components | `@:railsTemplateAst` methods returning inline HHX | Keep HHX as default. Template refs, slots, locals, model fields, and route helpers should be typed refs instead of repeated strings. | Medium; parser/helper coverage is still expanding. |
...
| Generators | Ruby/Rails-native `bin/rails generate hxruby:*` wrappers | Keep Rails-native generator UX. Use Haxe or Haxe->Ruby dogfooding only after Ruby generator contracts are stable. | Low; app-facing Rails UX should not regress. |
