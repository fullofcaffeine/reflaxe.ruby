...
| HHX templates/components | `@:railsTemplateAst` methods returning inline HHX | Keep HHX as default. Template refs, slots, locals, model fields, route helpers, and view-local presentation helpers should be typed refs/functions instead of repeated strings. Follow `railshx-view-local-helpers-design.md`: static scalar helpers plus same-class `HtmlNode` markup helpers are the current typed local-helper contract. | Medium; parser/helper coverage is still expanding. |
