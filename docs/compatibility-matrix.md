...
| Typed ActionMailer mailers | `@:railsMailer`, `MailerMacro.mailHtml/mailText/mailMultipart`, `Template<TLocals>` | Initial typed mailer/template slice plus generated Rails runtime lane for multipart bodies, headers, string attachments, and `deliver_now` |
| Typed ActiveStorage attachments | `@:hasOneAttached`, `@:hasManyAttached`, `Model.attachments.*` | Initial typed refs plus generated Rails runtime lane for signed-ID attach, download/read, and purge on one/many attachments |
...
| Haxe-owned route DSL | `@:railsRoutes` + `static final routes = { root(...); get(...); resources(...); }` | Initial slice: root, verb routes, typed controller/action refs, checked route aliases, resources/resource `only`/`except`/`param`, nested `member`/`collection`, `namespace`, `scope`, `controller`, and generated `config/routes.rb` |
