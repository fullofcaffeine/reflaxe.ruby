...
| Typed ActiveStorage attachments | `@:hasOneAttached`, `@:hasManyAttached`, `Model.attachments.*`, `Attachable`, `Attachables` | Initial typed refs plus generated Rails runtime lane for signed-ID attach, typed hash attachables, download/read, and purge on one/many attachments |
