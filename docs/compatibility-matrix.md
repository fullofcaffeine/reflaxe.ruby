...
| Typed server-side Turbo streams | `TurboStreams.append/prepend/before/after/replace/update/remove/refresh`, `broadcast*To`, `StreamTarget`, `StreamName<TPayload>`, `TurboRequestId`, `Template<TLocals>` | Rails-native `turbo_stream.*`/`Turbo::StreamsChannel` slice; targetless refresh, optional typed request-ID correlation, and synchronous broadcast runtime-proven |
