...
| Typed ActiveRecord relation chain | `Todo.none().reverseOrder().select(Todo.f.title).distinct().where({completed: false}).rewhere({status: "done"}).or(Todo.where({completed: true})).merge(Todo.where({status: "open"})).order(Todo.f.title.asc()).reorder(Order.many([Todo.f.id.desc(), Todo.f.title.asc()])).offset(20).limit(10) : Relation<Todo, criteria>`, `Todo.pluck(Todo.f.title) : Array<String>`, `Todo.maximum(Todo.f.id) : Null<Int>`, `Todo.sum(Todo.f.userId) : Int`, `Todo.average(Todo.f.userId) : Null<Float>` inferred | Initial query slice |
