...
| Typed ActionMailer mailers | `@:railsMailer`, `@:railsMailerParams`, `@:railsMailerPreview`, `MailerMacro.mailHtml/mailText/mailMultipart`, `Template<TLocals>`, `MailParam<T>`, `MessageDelivery` | Initial typed mailer/template/preview slice plus generated Rails runtime lane for multipart bodies, headers, string attachments, `deliver_now`, typed parameterized `.with(...)` params, and preview loading |
