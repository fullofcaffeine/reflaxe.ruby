...
| Typed Rails migrations | `@:railsMigration({models: [...], knownModels: [...], externalTables: [...]})` + snapshot `MigrationOperation` values | Model-derived create-table compatibility plus production-preferred snapshot operations: `CreateTable`, `Column`, `Reference` with named foreign keys, `Index`, checked column bounds, idempotent add/remove columns, named/idempotent indexes and named/idempotent index removal, named foreign keys and named foreign-key removal, `AddReference`, `AddCompositeIndex`, `RemoveCompositeIndex`, safe named/idempotent check constraints, rename/change-null, reversible destructive ops, migration version metadata, and explicit SQL/data rollback |
