...
| Typed ActiveRecord relation chain | `Todo.where({completed: false}).where(...).order(Todo.f.title.asc()).offset(20).limit(10) : Relation<Todo, criteria>` inferred | Initial query slice |
