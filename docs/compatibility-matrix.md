...
| Rails runtime evidence | `8.1.3.1` | Exact locked lane | The committed reference lock and canonical runtime lanes exercise Rails `8.1.3.1` with SQLite. Rails 8.1 receives upstream security support through 2027-10-10. |
...
`8.1.3.1` patch. Rails 7 and future Rails lines may work because RailsHx emits
...
| Pure RubyHx Haxe-first and Ruby-origin contract | `examples/rubyhx_cli`, `examples/ruby_callable_abi`, and handwritten Ruby consumers | Implemented with a multi-file library/CLI, filesystem and typed-JSON runtime paths, failure exit codes, negative Haxe typing, isolated candidate-ZIP consumption, native callable ABI coverage, and direct handwritten Ruby calls into generated APIs |
...
| Migration rollback safeguards | Name-only index/foreign-key removals require `Reversible(up, down)`. Validation-only `ChangeTable` values stay separate. Generic generator `--force` cannot replace Rails-owned Ruby migrations. | Compiler and generator enforced. Real Rails apply/rollback/reapply gate. |
...
| Typed server-side Turbo streams | `TurboStreams.append/prepend/before/after/replace/update/remove/refresh`, synchronous and delayed `broadcast*To`, `broadcastRefreshLaterTo`, `StreamTarget`, `StreamName<TPayload>`, `TurboRefreshOptions`, `TurboRequestId`, `Template<TLocals>` | Rails-native `turbo_stream.*`/`Turbo::StreamsChannel` slice; targetless refresh, closed replace/morph and reset/preserve behavior, optional typed request-ID correlation, synchronous broadcasts, debounced refreshes, and six ActionBroadcastJob render broadcasts runtime-proven |
...
| Typed ActionCable channels/subscriptions | `@:railsChannel`, `Channel<TParams, TPayload>`, generated `MyChannel.client:ChannelRef<TParams, TPayload>`, `Stream<TPayload>`, `SubscriptionParam<T>` | Stock Haxe JS and Genes client compile/runtime/negative smoke plus generated Rails `ActionCable::Channel::TestCase` runtime lane |
...
| Rails app install generator | `bin/rails generate hxruby:install` / `rake hxruby:gen:app` | Implemented with typed starter controller, HHX layout/page, Haxe-owned root route, client JS, CSS/importmap, `hxruby:start`, and the change-aware `bin/railshx-dev` / `hxruby:dev` loop (`hxruby:start:watch` alias retained) |
