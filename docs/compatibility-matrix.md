...
| Typed ActionMailer mailers | `@:railsMailer`, `MailerMacro.mailHtml/mailText/mailMultipart`, `Template<TLocals>`, `MailParam<T>`, `MessageDelivery` | Initial typed mailer/template slice plus generated Rails runtime lane for multipart bodies, headers, string attachments, `deliver_now`, and typed parameterized `.with(...)` params |
