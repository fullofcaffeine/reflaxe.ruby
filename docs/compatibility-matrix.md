...
| Typed ActiveRecord relation chain | `Todo.all().distinct().where({completed: false}).rewhere({status: "done"}).order(Todo.f.title.asc()).reorder(Todo.f.id.desc()).offset(20).limit(10) : Relation<Todo, criteria>`, `Todo.pluck(Todo.f.title) : Array<String>`, `Todo.maximum(Todo.f.id) : Null<Int>` inferred | Initial query slice |
