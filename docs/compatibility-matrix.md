...
| ActiveJob lifecycle | `static final lifecycle = { queueAs("mailers"); retryOn(StandardError, {attempts: 3}); discardOn(DeserializationError); }` | Initial contextual lifecycle DSL for `queue_as`, `retry_on`, and `discard_on`; legacy class metadata remains compatibility-only |
