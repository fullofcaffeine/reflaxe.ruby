...
| Controller request/response facades | `request().requestMethod()`, `request().format().json()`, `request().variant().phone()`, `response().status()` | Initial typed extern facade slice, including MIME format and variant inquirers |
