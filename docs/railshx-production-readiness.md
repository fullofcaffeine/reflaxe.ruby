...
| Performance and resource behavior | A small representative set measures compiler and generated-program startup time, peak memory, and output size on the canonical production lane. Broad caps block runaway regressions; ordinary hosted-runner variance remains reviewable evidence. Runtime, request, job, browser, and production correctness stay with their existing mandatory lanes. Any latency, throughput, allocation, or comparative-speed claim requires a purpose-built workload before publication. |
...
| `haxe_ruby-hjm` | Typed Ruby core and stdlib catalog | The packaged inventory distinguishes core, stdlib, default gems, bundled gems, and platform-specific APIs, accounts for every maintained `ruby.*` facade, adds the reviewed-RBS URI slice, and packages a strict deterministic precise-or-omitted RBS extern generator. Broader RBS shapes and later library domains remain post-1.0 breadth unless a narrower marketed workflow depends on them. |
