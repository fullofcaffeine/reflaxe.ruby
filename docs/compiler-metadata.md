...
| `@:railsDomTargets(target, ...)` | RailsHx-owned `@:railsTemplateAst` view; one or more compile-time String tokens. | Proves each declared token appears as a static `id` in that view's structural HHX tree. Missing or dynamic ids fail closed; Rails-owned ERB uses `StreamTarget.existing(template, target)` instead. |
...
| `@:hotwireContractGenerated` | The Hotwire build macro marks inline accessors generated from `@:hotwireContract` or `@:hotwireHooks` for diagnostics and inspection. Application code authors the public declaration metadata, not this marker. |
