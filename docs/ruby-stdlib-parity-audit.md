...
| `covered-upstream-fallback` | 15 | The Ruby lane covers the surface while using upstream Haxe std source, sometimes over lower-level Ruby-owned dependencies. |
| `upstream-fallback-candidate` | 11 | No Ruby override is indicated yet. Add a fixture or smoke before promoting to covered. |
...
`haxe.crypto.Sha256`, `haxe.ds.GenericStack`, `haxe.io.BytesBuffer`, and
...
1. Promote one upstream-fallback candidate such as `haxe.Template`,
   `haxe.DynamicAccess`, or `haxe.zip.Compress` through
