...
| `covered-upstream-fallback` | 14 | The Ruby lane covers the surface while using upstream Haxe std source, sometimes over lower-level Ruby-owned dependencies. |
| `upstream-fallback-candidate` | 12 | No Ruby override is indicated yet. Add a fixture or smoke before promoting to covered. |
...
`haxe.crypto.Sha256`, `haxe.io.BytesBuffer`, and `haxe.io.Path` are covered
...
1. Promote one upstream-fallback candidate such as `haxe.Template`,
   `haxe.DynamicAccess`, or `haxe.ds.GenericStack` through
