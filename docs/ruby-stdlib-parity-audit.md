...
| `covered-upstream-fallback` | 16 | The Ruby lane covers the surface while using upstream Haxe std source, sometimes over lower-level Ruby-owned dependencies. |
| `upstream-fallback-candidate` | 10 | No Ruby override is indicated yet. Add a fixture or smoke before promoting to covered. |
...
`haxe.Template`, `haxe.crypto.Sha256`, `haxe.ds.GenericStack`,
...
1. Promote one upstream-fallback candidate such as `haxe.DynamicAccess`,
   `haxe.ds.BalancedTree`, or `haxe.zip.Compress` through
