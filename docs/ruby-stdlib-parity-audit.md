...
| `covered-upstream-fallback` | 18 | The Ruby lane covers the surface while using upstream Haxe std source, sometimes over lower-level Ruby-owned dependencies. |
| `upstream-fallback-candidate` | 8 | No Ruby override is indicated yet. Add a fixture or smoke before promoting to covered. |
...
1. Promote one upstream-fallback candidate such as `haxe.ds.BalancedTree`,
   `haxe.Log`, or `haxe.zip.Compress` through
