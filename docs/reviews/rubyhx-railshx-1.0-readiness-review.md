...
8. “Supported compiler/std surface” is dependency-closed for RailsHx. Every
   marketed component slice must prove the Haxe/std, compiler/runtime, direct
   Ruby facade, Rails/gem, generated-artifact, and target-execution dependencies
   it actually uses. This does not require the whole Haxe or Ruby stdlib.
...
**Maintainer resolution:** The review correctly found a missing repeatable
measurement, but its requested workload set duplicated existing correctness
lanes and implied false precision on shared runners. The bounded stable contract
uses three clean compiler workloads, generated CLI startup, and prepared Rails
production boot. It records three cold/repeat samples, wall time, peak RSS,
generated bytes/files, variance, toolchain/runner identity, and exact source SHA.
Existing Rails runtime, browser, and production lanes continue to prove request,
job, asset, and deployment behavior. No latency, throughput, allocation, or
comparative-speed claim is made.

**Acceptance evidence:**

1. `npm run benchmark:stable` emits machine-readable and human results.
2. The canonical production job records exact source, toolchain, runner, and
   workload metadata; release-candidate evidence links that exact run.
3. Three samples expose ordinary variance without treating it as failure.
4. Broad absolute time, peak-RSS, byte, and file-count caps block runaway work.
5. Public docs keep every number workload-scoped and make no speed comparison.
