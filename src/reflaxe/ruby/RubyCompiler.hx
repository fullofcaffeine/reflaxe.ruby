...
typedef RailsTemplateScope = {
	...
	viewHelpers:Map<String, RailsTemplateHelper>,
	?formBuilderName:String
}

typedef RailsTemplateHelper = {
	owner:String,
	field:ClassField,
	args:Array<ClassFuncArg>,
	body:TypedExpr,
	ret:Type
}
...
class RubyCompiler extends GenericCompiler<RubyFile, RubyFile, RubyExpr, RubyFile, RubyFile> {
	...
	var railsTestArtifacts:RailsTestArtifacts = new RailsTestArtifacts();
	var railsMailerPreviewArtifacts:RailsMailerPreviewArtifacts = new RailsMailerPreviewArtifacts();
	var requireRegistry:RequireRegistry = new RequireRegistry();
	var buildContext:RubyBuildContext;
	var didEmitMain:Bool = false;

	static inline final RAILS_GENERATED_SUPPORT_ROOT = "app/lib/railshx/generated";
	...
	override public function onCompileStart():Void {
		...
		railsTestArtifacts.reset();
		railsMailerPreviewArtifacts.reset();
	...
	override public function onCompileEnd():Void {
		...
		var paths = [];
		if (needsCoreRuntime) {
			paths.push(outputRelativePath("hxruby/core.rb", true));
		}
		if (needsDataDefine) {
		...
		var lines = RubyOutputLayout.runnerPrelude(requireRegistry.requireValues(), quoteRubyStringForCode);
	...
	public function compileClassImpl(classType:ClassType, varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>):Null<RubyFile> {
		...
		if (hasRubyNoEmit(classType.meta)) {
			if (isRailsArtifactType(classType)) {
				Context.error("@:rubyNoEmit marks a type as compile-time-only, so it cannot also declare a Rails artifact metadata such as @:railsModel, @:railsController, @:railsTemplate, or @:railsMigration.",
					classType.pos);
			}
			return null;
		}
		...
		setRubyOutputPathForClass(classType);
		...
		if (isRailsControllerType(classType)) {
		...
		if (isRailsMailerType(classType)) {
		...
		addRuntimeCoreRequireIfNeeded(moduleRequires, classType, hxrubyCallsBefore);
	...
	static function rubyClassDeclaration(classType:ClassType, body:Array<RubyStatement>):RubyStatement {
		var constant = RubyOutputLayout.ownedClassConstant(classType.pack, classType.name);
	...
	static function rubyRequireRelativePathFrom(fromPack:Array<String>, targetPack:Array<String>, targetName:String):String {
		...
		targetParts.push(RubyOutputLayout.outputFileName(targetPack, targetName));
		var common = 0;
	...
	**/
	...
	static function normalizeRailsScopeBody(line:String, classType:ClassType):String {
		var prefix = rubyClassConstantPath(classType) + ".";
	...
	static function shouldSuppressRailsRubyType(classType:ClassType, varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>):Bool {
		...
		if (isCompilerOwnedSupportType(classType.pack, classType.name)) {
			return true;
		}
		return isRailsInlineOnlySupportType(varFields, funcFields);
	}

	static function isRailsArtifactType(classType:ClassType):Bool {
		return hasMeta(classType.meta, ":railsTest")
			|| hasMeta(classType.meta, ":railsMailerPreview")
			|| hasMeta(classType.meta, ":railsRoutes")
			|| hasMeta(classType.meta, ":railsMigration")
			|| hasMeta(classType.meta, ":railsTemplate")
			|| hasMeta(classType.meta, ":railsModel")
			|| isRailsControllerType(classType)
			|| isRailsMailerType(classType)
			|| hasMeta(classType.meta, ":railsJob")
			|| hasMeta(classType.meta, ":railsChannel")
			|| hasMeta(classType.meta, ":railsCableConnection");
	}

	static function isRailsControllerType(classType:ClassType):Bool {
		return hasMeta(classType.meta, ":railsController") || hasMeta(classType.meta, ":railsApplicationController");
	}

	static function isRailsMailerType(classType:ClassType):Bool {
		return hasMeta(classType.meta, ":railsMailer") || inheritedRailsMailerSuperclass(classType) != null;
	}

	static function inheritedRailsMailerSuperclass(classType:ClassType):Null<String> {
		var superClass = classType.superClass;
		while (superClass != null) {
			var superType = superClass.t.get();
			var superclass = metaStringParam(superType.meta, ":railsMailerSuperclass", 0);
			if (superclass != null) {
				return superclass;
			}
	...
	static function isCompilerOwnedSupportType(pack:Array<String>, typeName:String):Bool {
		if (isStdRubyType(pack, typeName)) {
			return true;
		}
	...
	static function isRailsInlineOnlySupportType(varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>):Bool {
		...
		for (field in funcFields) {
			if (!field.isStatic || !isInlineMethod(field)) {
				return false;
			}
		}
		return true;
	...
	static function isInlineMethod(field:ClassFuncData):Bool {
		return switch (field.kind) {
	...
	function setRubyOutputPath(pack:Array<String>, typeName:String):Void {
		setOutputFileName(RubyOutputLayout.outputFileName(pack, typeName));
		setOutputFileDir(outputRelativeDir(RubyNaming.fileDir(pack), !isStdRubyType(pack, typeName)));
		rememberRubyPath(pack, typeName);
	}

	function setRubyOutputPathForClass(classType:ClassType):Void {
		var railsNativePath = railsNativeRubyPath(classType);
		if (railsNativePath != null) {
			setRubyOutputFilePath(railsNativePath, true);
			return;
		}
		setRubyOutputPath(classType.pack, classType.name);
	}

	function setRubyOutputFilePath(path:String, appOwned:Bool):Void {
		var normalized = normalizeRubyOutputFilePath(path);
		var withoutExt = StringTools.endsWith(normalized, ".rb") ? normalized.substr(0, normalized.length - 3) : normalized;
		var slash = withoutExt.lastIndexOf("/");
		if (slash < 0) {
			setOutputFileName(withoutExt);
			setOutputFileDir(null);
		} else {
			setOutputFileName(withoutExt.substr(slash + 1));
			setOutputFileDir(withoutExt.substr(0, slash));
		}
		rememberRubyPathFromPath(StringTools.endsWith(normalized, ".rb") ? normalized : normalized + ".rb", appOwned);
	}

	function rememberRubyPath(pack:Array<String>, typeName:String):Void {
		var path = RubyOutputLayout.outputFileName(pack, typeName) + ".rb";
		var dir = outputRelativeDir(RubyNaming.fileDir(pack), !isStdRubyType(pack, typeName));
		if (dir != null && dir != "") {
			path = dir + "/" + path;
		}
		rememberRubyPathFromPath(path, !isStdRubyType(pack, typeName));
	}

	function rememberRubyPathFromPath(path:String, appOwned:Bool):Void {
		if (emittedRubyPaths.indexOf(path) == -1) {
			emittedRubyPaths.push(path);
		}
		if (appOwned && emittedAppRubyPaths.indexOf(path) == -1) {
			emittedAppRubyPaths.push(path);
		}
	}

	function railsNativeRubyPath(classType:ClassType):Null<String> {
		if (!buildContext.usesRailsNativeLayout()) {
			return null;
		}
		var fileName = RubyNaming.fileName(classType.name) + ".rb";
		if (hasMeta(classType.meta, ":railsModel")) {
		...
		if (hasMeta(classType.meta, ":railsChannel")) {
			return "app/channels/" + fileName;
		}
		if (hasMeta(classType.meta, ":railsCableConnection")) {
			return "app/channels/application_cable/connection.rb";
		}
		return null;
	}

	static function normalizeRubyOutputFilePath(path:String):String {
		var normalized = StringTools.trim(path == null ? "" : path).split("\\").join("/");
	...
	function railsArtifactStatement(classType:ClassType, statement:RubyStatement):RubyStatement {
		return buildContext.usesRailsNativeLayout() ? statement : wrapInModules(classType.pack, statement);
	}

	function addRuntimeCoreRequireIfNeeded(moduleRequires:RequireRegistry, classType:ClassType, callsBefore:Int):Void {
		if (hxrubyCallCount <= callsBefore) {
			return;
		}
		if (buildContext.usesRailsNativeLayout()) {
			var sourcePath = railsNativeRubyPath(classType);
			if (sourcePath == null) {
				return;
			}
		...
		moduleRequires.addRequireRelative(rubyRequireRelativePathFrom(classType.pack, ["hxruby"], "core"));
	}

	static function relativeRequirePath(fromFile:String, toFile:String):String {
		var fromDir = pathDir(normalizeRubyOutputFilePath(fromFile));
		var toPath = normalizeRubyOutputFilePath(StringTools.endsWith(toFile, ".rb") ? toFile.substr(0, toFile.length - 3) : toFile);
		var fromParts = splitPath(fromDir);
		var toParts = splitPath(toPath);
		var common = 0;
		...
		for (i in common...toParts.length) {
			out.push(toParts[i]);
		}
		return out.length == 0 ? "." : out.join("/");
	}

	static function pathDir(path:String):String {
		var slash = path.lastIndexOf("/");
		return slash < 0 ? "" : path.substr(0, slash);
	}

	static function splitPath(path:String):Array<String> {
		return [for (part in path.split("/")) if (part != "") part];
	}

	function railsControllerSuperclass(classType:ClassType):String {
		if (isRailsApplicationController(classType)) {
			return "ActionController::Base";
		}
		if (classType.superClass != null) {
			var superclass = classType.superClass.t.get();
			if (fullTypeName(superclass.pack, superclass.name) != "rails.action_controller.Base") {
				return rubyNativeName(superclass.meta) ?? rubyClassConstantPath(superclass);
			}
		}
		return buildContext.usesRailsNativeLayout() ? "ApplicationController" : "ActionController::Base";
	}

	static function isRailsApplicationController(classType:ClassType):Bool {
		return hasMeta(classType.meta, ":railsApplicationController")
			|| (classType.pack.length == 1 && classType.pack[0] == "controllers" && classType.name == "ApplicationController");
	}
	...
	function compileRailsControllerImpl(classType:ClassType, varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>,
		...
		var hxrubyCallsBefore = hxrubyCallCount;
		validateRequiredControllerFilters(varFields, funcFields);
		...
		var lines = [
			"class " + RubyNaming.toConstantName(classType.name) + " < " + railsControllerSuperclass(classType)
		];
		for (line in body) {
			lines.push("  " + line);
		}
		...
		addRuntimeCoreRequireIfNeeded(moduleRequires, classType, hxrubyCallsBefore);
		statements = statements.concat(requirePreludeStatements(moduleRequires));
		statements.push(railsArtifactStatement(classType, RubyRawStatement(lines.join("\n"))));
		return {
			modulePath: classType.pack == null ? [] : classType.pack.copy(),
			statements: statements
		};
	...
	function compileRailsMailerImpl(classType:ClassType, varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>,
		...
		var superclass = inheritedRailsMailerSuperclass(classType) ?? "ActionMailer::Base";
		var lines = ["class " + RubyNaming.toConstantName(classType.name) + " < " + superclass];
		for (line in body) {
			lines.push("  " + line);
		}
		lines.push("end");
	...
	function compileRailsCableConnectionImpl(classType:ClassType, varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>,
		...
		if (buildContext.usesRailsNativeLayout()) {
			lines.push("module ApplicationCable");
			lines.push("  class Connection < ActionCable::Connection::Base");
			for (line in body) {
				lines.push("    " + line);
			}
			lines.push("  end");
			...
			lines.push("class " + RubyNaming.toConstantName(classType.name) + " < ActionCable::Connection::Base");
			for (line in body) {
				lines.push("  " + line);
			}
			lines.push("end");
	...
	static function validateRequiredControllerFilters(varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>):Void {
		if (!Context.defined("railshx_strict_required_filters")) {
			return;
		}
		var protectedFiltersByAction = protectedControllerFiltersByAction(varFields, funcFields);
		for (field in funcFields) {
			if (field.isStatic || field.expr == null || hasMeta(field.field.meta, ":rubyExternStub")) {
				continue;
			}
			var action = field.field.name;
			var filters = protectedFiltersByAction.get(action);
			if (filters == null) {
				filters = new Map();
			}
			iterTypedExprRecursive(field.expr, function(expr:TypedExpr) {
				var requiredFilter = requiredControllerFilter(expr);
				if (requiredFilter == null) {
					return;
				}
				if (!filters.exists(requiredFilter)) {
					Context.error('Controller action "$action" calls a value requiring filter "$requiredFilter" and requires a matching beforeAction guard, but none covers the action. Add the guard, narrow the lifecycle only/except options, or use the nullable companion API.',
						expr.pos);
				}
			});
		}
	...
	static function protectedControllerFiltersByAction(varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>):Map<String, Map<String, Bool>> {
			...
			switch (decl.kind) {
				case "before_action":
					for (action in actions) {
						...
						var filters = out.get(action);
						if (filters == null) {
							filters = new Map();
							out.set(action, filters);
						}
						filters.set(decl.method, true);
					}
	...
	static function requiredControllerFilter(expr:TypedExpr):Null<String> {
		return switch (unwrapTypedExpr(expr).expr) {
			case TCall(callee, params):
				...
				if (info == null) {
					...
					var direct = metaStringParam(info.field.meta, ":railsRequiresFilter", 0);
					if (direct != null) {
						validateRubyMethodName(direct, info.field.pos, "@:railsRequiresFilter");
						direct;
					} else {
						var argIndex = metaIntParam(info.field.meta, ":railsRequiresFilterArg", 0);
						if (argIndex == null) {
							null;
						} else if (argIndex < 0 || argIndex >= params.length) {
							Context.error("@:railsRequiresFilterArg points outside the call arguments.", info.field.pos);
							null;
						} else {
							var value = extensionStringLiteral(params[argIndex], "Required controller filter must be a constant String.");
							validateRubyMethodName(value, params[argIndex].pos, "required controller filter");
							value;
						}
					}
				}
	...
	static function validateRubyMethodName(value:String, pos:Position, context:String):Void {
		if (!~/^[a-z_][A-Za-z0-9_]*[!?=]?$/.match(value)) {
			Context.error('$context must be a safe Ruby method name.', pos);
		}
	}
	...
	function compileRailsModelImpl(classType:ClassType, varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>,
		...
		body = body.concat(rubyExtensionLines(classType.meta, classType));
		body = body.concat(railsClassMacroLines(classType));
		for (field in varFields) {
		...
		var modelSuperclass = buildContext.usesRailsNativeLayout() ? "ApplicationRecord" : "::ApplicationRecord";
		var lines = ["class " + RubyNaming.toConstantName(classType.name) + " < " + modelSuperclass];
		for (line in body) {
			lines.push("  " + line);
		}
		lines.push("end");
	...
	static function railsClassMacroLines(classType:ClassType):Array<String> {
		var lines:Array<String> = [];
		for (entry in classType.meta.extract(":railsClassMacro")) {
			if (entry.params == null || entry.params.length != 2) {
				Context.error("@:railsClassMacro expects a Ruby method name and an array of symbol names.", entry.pos);
				continue;
			}
			var method = switch (entry.params[0].expr) {
				case EConst(CString(value, _)):
					validateRubyMethodName(value, entry.params[0].pos, "@:railsClassMacro method");
					value;
				case _:
					Context.error("@:railsClassMacro method must be a String literal.", entry.params[0].pos);
					"";
			}
			var symbols = switch (entry.params[1].expr) {
				case EArrayDecl(values):
					[
						for (value in values)
							switch (value.expr) {
								case EConst(CString(name, _)):
									if (!isSafeRubyIdentifier(name)) {
										Context.error("@:railsClassMacro symbol names must be safe Ruby identifiers.", value.pos);
									}
									name;
								case _:
									Context.error("@:railsClassMacro symbols must be String literals.", value.pos);
									"";
							}
					];
				case _:
					Context.error("@:railsClassMacro second argument must be an Array<String> literal.", entry.params[1].pos);
					[];
			}
			if (method != "" && symbols.length > 0) {
				lines.push(method + " " + [for (symbol in symbols) rubySymbolLiteral(symbol)].join(", "));
			}
		}
		return lines;
	}
	...
	static function compileFunctionBody(expr:TypedExpr):Array<RubyStatement> {
		return switch (expr.expr) {
			...
			case _:
				normalizeRubyStatements([compileStatement(expr)]);
		}
	}
	...
	static function compileStatementList(exprs:Array<TypedExpr>):Array<RubyStatement> {
		...
		return normalizeRubyStatements(out);
	}

	static function normalizeRubyStatements(statements:Array<RubyStatement>):Array<RubyStatement> {
		var out = [for (statement in statements) if (!isRubyNoopStatement(statement)) statement];
		if (out.length == 0) {
	...
	static function isRubyNoopStatement(statement:RubyStatement):Bool {
	...
	**/
	...
	static function compileSpecialCall(callee:TypedExpr, params:Array<TypedExpr>):Null<RubyExpr> {
		...
		var extensionExpr = compileRubyExtensionExpr(callee, params);
		...
		var stringStaticCall = compileStringStaticCall(info, params);
		if (stringStaticCall != null) {
			return stringStaticCall;
		}
		...
		return switch [info.owner, info.name] {
			...
			case ["Std", "string"]:
				compileRubyStringifyParam(params[0]);
			...
			case ["Sys", "print"]:
				RubyCall(null, "print", [compileRubyStringifyParam(params[0])]);
			case ["Sys", "args"]:
				RubyRawExpr("ARGV");
			case ["Sys", "getEnv"]:
			...
			case ["StringTools", "htmlEscape"]:
				rubyStringToolsHtmlEscape(params);
			...
			case ["StringTools", "hex"]:
				rubyStringToolsHex(params);
			case ["StringTools", "urlEncode"]:
				RubyRawExpr("URI.encode_www_form_component(" + printParam(params, 0) + ".to_s)");
	...
	static function rubyStringToolsHtmlEscape(params:Array<TypedExpr>):RubyExpr {
		var value = printParam(params, 0);
		var quotes = params.length > 1 ? printParam(params, 1) : "false";
		return RubyRawExpr("(begin out = "
			+ value
			+ ".to_s.gsub('&', '&amp;').gsub('<', '&lt;').gsub('>', '&gt;'); "
			+ quotes
			+ " ? out.gsub(34.chr, '&quot;').gsub(39.chr, '&#039;') : out end)");
	}

	static function rubyStringToolsReplace(params:Array<TypedExpr>):RubyExpr {
		return
			RubyRawExpr("(->(string, needle, by) { needle.empty? ? (by.empty? ? string : string.each_char.to_a.join(by)) : string.gsub(needle) { by } }).call("
			+ printParam(params, 0)
			+ ".to_s, "
			+ printParam(params, 1)
			+ ".to_s, "
	...
	static function rubyStringToolsHex(params:Array<TypedExpr>):RubyExpr {
		var digits = params.length > 1 ? printParam(params, 1) : "nil";
		return RubyRawExpr("(->(value, digits) { out = (value.to_i & 0xffffffff).to_s(16).upcase; digits.nil? ? out : out.rjust(digits.to_i, '0') }).call("
			+ printParam(params, 0)
			+ ", "
			+ digits
			+ ")");
	...
	static function markCoreRuntimeUse():Void {
		needsCoreRuntime = true;
		hxrubyCallCount++;
	}

	static function hasHXRubyRuntimeReference(code:String):Bool {
		return code.indexOf("HXRuby.") != -1 || code.indexOf("HXRuby::") != -1;
	}
	...
	**/
	...
	static function compileStringStaticCall(info:{owner:String, name:String}, params:Array<TypedExpr>):Null<RubyExpr> {
		if (info.owner != "String") {
			return null;
		}
		return switch info.name {
			case "fromCharCode" if (params.length == 1):
				RubyRawExpr("(" + printParam(params, 0) + ").chr(Encoding::UTF_8)");
			case _:
				null;
		}
	}
		...
		return switch (info.name) {
			...
			case "broadcastRefreshTo" | "broadcastRefreshLaterTo" if (params.length == 1 || params.length == 2):
				...
				if (params.length == 2 && !isNullLiteral(params[1])) {
					...
					if (options == null) {
						Context.error("TurboStreams." + info.name + " options must be a typed object literal.", params[1].pos);
					} else {
						for (option in options) {
							args.push(RubyKeywordArgument(option.rubyName, compileExpr(option.value)));
						}
				...
				var rubyName = info.name == "broadcastRefreshTo" ? "broadcast_refresh_to" : "broadcast_refresh_later_to";
				RubyCallableCall(RubyConstantPath("Turbo::StreamsChannel"), rubyName, args);
			case _:
				null;
		}
	}
	...
	static function activeRecordModelRubyPathFromType(type:haxe.macro.Type):Null<String> {
		return switch (TypeTools.follow(type)) {
			case TInst(ref, _):
				...
				rubyNativeName(classType.meta) ?? rubyClassConstantPath(classType);
			case _:
				null;
		}
	}
	...
	**/
	...
	static function compileRubyInjection(callee:TypedExpr, params:Array<TypedExpr>):Null<RubyExpr> {
		...
		if (hasHXRubyRuntimeReference(code)) {
			markCoreRuntimeUse();
		}
	...
	/**
		Lowers a compiler-erased extension expression after its owning facade has
		validated and frozen the Ruby template. Requiring both `@:rubyNoEmit` and
		`@:rubyAllowRaw` keeps this handoff explicit; application code remains
		subject to the normal raw-injection policy.
	**/
	static function compileRubyExtensionExpr(callee:TypedExpr, params:Array<TypedExpr>):Null<RubyExpr> {
		var code = rubyExtensionExprCode(callee, params, printInlineExpr);
		if (code == null) {
			return null;
		}
		if (hasHXRubyRuntimeReference(code)) {
	...
	static function rubyExtensionExprCode(callee:TypedExpr, params:Array<TypedExpr>, render:TypedExpr->String):Null<String> {
		return switch (unwrapTypedExpr(callee).expr) {
			case TField(_, FStatic(classRef, fieldRef)):
				var owner = classRef.get();
				var field = fieldRef.get();
				var contract = metadataObject(field.meta, ":rubyExtensionExpr");
				if (contract == null) {
					null;
				} else {
					if (!hasRubyNoEmit(owner.meta) || !hasMeta(owner.meta, ":rubyAllowRaw")) {
						Context.error("@:rubyExtensionExpr requires an owning compile-time facade marked with both @:rubyNoEmit and @:rubyAllowRaw.",
							field.pos);
					}
					validateMetadataObjectKeys(contract, ["schema", "template", "templateArg", "valueStart"], ":rubyExtensionExpr", field.pos);
					var schema = metadataObjectInt(contract, "schema", field.pos, ":rubyExtensionExpr");
					if (schema != 1) {
						Context.error('@:rubyExtensionExpr unsupported schema $schema; expected schema 1.', field.pos);
					}
					var literalTemplate = metadataObjectOptionalString(contract, "template", field.pos, ":rubyExtensionExpr");
					var templateArg = metadataObjectOptionalInt(contract, "templateArg", field.pos, ":rubyExtensionExpr");
					if ((literalTemplate == null) == (templateArg == null)) {
						Context.error("@:rubyExtensionExpr requires exactly one of template or templateArg.", field.pos);
					}
					var template = literalTemplate;
					if (templateArg != null) {
						if (templateArg < 0 || templateArg >= params.length) {
							Context.error("@:rubyExtensionExpr templateArg is outside the call arguments.", field.pos);
						}
						template = extensionStringLiteral(params[templateArg], "@:rubyExtensionExpr template argument must be a constant String.");
					}
					var valueStart = metadataObjectOptionalInt(contract, "valueStart", field.pos, ":rubyExtensionExpr");
					if (valueStart == null) {
						valueStart = templateArg == null ? 0 : templateArg + 1;
					}
					if (valueStart < 0 || valueStart > params.length) {
						Context.error("@:rubyExtensionExpr valueStart is outside the call arguments.", field.pos);
					}
					~/{(\d+)}/g.map(template, ereg -> {
						var index = Std.parseInt(ereg.matched(1));
						var paramIndex = index == null ? -1 : valueStart + index;
						if (paramIndex < valueStart || paramIndex >= params.length) {
							Context.error('@:rubyExtensionExpr placeholder ${ereg.matched(0)} has no matching value argument.', field.pos);
							return "nil";
						}
						return render(params[paramIndex]);
					});
				}
			case _:
				null;
	...
	static function extensionStringLiteral(expr:TypedExpr, message:String):String {
		return switch (unwrapTypedExpr(expr).expr) {
			case TConst(TString(value)): value;
			case _:
	...
	**/
	...
	function setRailsAutoloadInitializer():Void {
		var root = buildContext.usesRailsNativeLayout() ? RAILS_GENERATED_SUPPORT_ROOT : buildContext.railsOutputRoot;
		var runtimeRoot = buildContext.usesRailsNativeLayout() ? RAILS_VENDOR_RUNTIME_ROOT : buildContext.railsOutputRoot + "/hxruby";
		var lines = [
			"# Generated by reflaxe.ruby",
			"hxruby_root = Rails.root.join(" + quoteRubyStringForCode(root) + ")",
			"hxruby_runtime_root = Rails.root.join(" + quoteRubyStringForCode(runtimeRoot) + ")",
			"Rails.autoloaders.main.ignore(hxruby_runtime_root) if defined?(Rails.autoloaders) && hxruby_runtime_root.exist?",
			buildContext.usesRailsNativeLayout() ? null : "Dir[hxruby_runtime_root.join(\"*.rb\")].sort.each { |path| require path }",
		...
		setRailsExtraFile("config/initializers/hxruby_autoload.rb", [for (line in lines) if (line != null) line].join("\n"), Context.currentPos());
	}
	...
	static function railsMigrationChangeTableOperation(table:String, optionsExpr:TypedExpr, allowIrreversible:Bool,
		...
		var validationOperationCount = 0;
		...
		if (!hasBody) {
			Context.error("@:railsMigration ChangeTable requires at least one typed column/default/null change, column/index rename, constraint, column/reference/index item, typed removal, or timestamp operation.",
				optionsExpr.pos);
		}
		...
		lines = lines.concat(bodyLines);
		lines.push("end");
		return validationOperationCount > 0 ? railsMigrationValidationOperation(lines, allowIrreversible,
			foreignKeys) : railsMigrationOperation(lines, foreignKeys);
	}
	...
	static function railsMigrationChangeTableForeignKeys(expr:TypedExpr, table:String,
			validation:Null<RailsMigrationValidationContext>):Array<{lines:Array<String>, toTable:String}> {
		return switch (unwrapTypedExpr(expr).expr) {
			case TArrayDecl(values):
				var out:Array<{lines:Array<String>, toTable:String}> = [];
				for (value in values) {
	...
	static function railsMigrationChangeTableForeignKey(expr:TypedExpr, table:String,
			validation:Null<RailsMigrationValidationContext>):{lines:Array<String>, toTable:String} {
		...
		if (toTableExpr == null) {
			...
			return {lines: ["t.foreign_key :invalid"], toTable: "invalid"};
		}
		if (optionsExpr == null) {
		...
		var info = railsMigrationForeignKeyDslInfo(optionsExpr, validation, table);
	...
	static function railsMigrationChangeTableUniqueConstraintInfo(expr:TypedExpr, table:String, validation:Null<RailsMigrationValidationContext>,
		...
		if (columnsExpr == null) {
			...
			return {name: "invalid", line: command + " [:invalid]"};
		}
		if (optionsExpr == null) {
	...
	static function railsMigrationChangeTableGuardedUniqueConstraints(expr:TypedExpr, table:String, validation:Null<RailsMigrationValidationContext>,
		...
		return switch (unwrapTypedExpr(expr).expr) {
			case TArrayDecl(values):
				var out:Array<String> = [];
				for (value in values) {
					var item = railsMigrationChangeTableUniqueConstraintInfo(value, table, validation, fieldName, command);
					out.push(guard + " t.unique_constraint_exists?(name: " + quoteRubyStringForCode(item.name) + ")");
					out.push("  " + item.line);
					out.push("end");
				}
				if (out.length == 0) {
	...
	static function railsMigrationChangeTableExclusionConstraints(expr:TypedExpr, fieldName:String, command:String):Array<String> {
		return switch (unwrapTypedExpr(expr).expr) {
			case TArrayDecl(values):
				var out:Array<String> = [];
				for (value in values) {
					out.push(railsMigrationChangeTableExclusionConstraint(value, fieldName, command));
				}
				if (out.length == 0) {
					Context.error('@:railsMigration ChangeTable $fieldName must not be empty.', expr.pos);
				}
	...
	static function railsMigrationChangeTableExclusionConstraint(expr:TypedExpr, fieldName:String, command:String):String {
	...
	static function railsMigrationChangeTableExclusionConstraintInfo(expr:TypedExpr, fieldName:String, command:String):{name:String, line:String} {
		...
		if (expressionExpr == null) {
			Context.error('@:railsMigration ChangeTable $fieldName entry requires expression.', expr.pos);
			return {name: "invalid", line: command + " \"\""};
		}
		if (optionsExpr == null) {
			Context.error('@:railsMigration ChangeTable $fieldName entry requires options.', expr.pos);
			return {name: "invalid", line: command + " \"\""};
	...
	static function railsMigrationChangeTableGuardedExclusionConstraints(expr:TypedExpr, fieldName:String, guard:String, command:String):Array<String> {
		return switch (unwrapTypedExpr(expr).expr) {
			case TArrayDecl(values):
				var out:Array<String> = [];
				for (value in values) {
					var item = railsMigrationChangeTableExclusionConstraintInfo(value, fieldName, command);
					out.push(guard + " t.exclusion_constraint_exists?(name: " + quoteRubyStringForCode(item.name) + ")");
					out.push("  " + item.line);
					out.push("end");
				}
				if (out.length == 0) {
	...
	static function railsMigrationIndexLock(expr:TypedExpr, label:String):String {
		return switch (unwrapTypedExpr(expr).expr) {
			case TField(_, FEnum(_, field)):
				railsMigrationIndexLockName(field.name, label, expr);
			case TCall({expr: TField(_, FEnum(_, field))}, _):
				railsMigrationIndexLockName(field.name, label, expr);
			case _:
				Context.error('@:railsMigration ${label} must be an IndexLock enum value.', expr.pos);
				"none";
		}
	}
	...
	static function railsMigrationForeignKeyDslOptions(expr:TypedExpr, validation:Null<RailsMigrationValidationContext>, fromTable:String,
		...
		return railsMigrationForeignKeyDslInfo(expr, validation, fromTable, allowIfNotExists).options;
	}
			...
			case TCall(callee, params):
				...
				if (info != null && info.owner == "rails.routing.RouteDecl") {
					switch (info.name) {
						...
						case "extension" if (params.length == 7):
							{
								kind: "extension",
								target: null,
								extension: {
									label: railsRouteString(params[0], "extension label"),
									line: railsRouteLine(params[1]),
									manifestJson: railsRouteString(params[2], "extension manifest"),
									topLevelOnly: railsRouteBool(params[3], "extension top-level flag"),
									group: railsRouteStringAllowEmpty(params[4], "extension group"),
									signature: railsRouteStringAllowEmpty(params[5], "extension signature"),
									split: railsRouteBool(params[6], "extension split flag")
								},
								verb: "",
								verbs: [],
								path: "",
	...
	static function railsRouteLine(expr:TypedExpr):String {
		var value = railsRouteString(expr, "extension Ruby line");
		if (value.indexOf("\n") != -1 || value.indexOf("\r") != -1) {
			Context.error("@:railsRoutes extension Ruby line must be a single line.", expr.pos);
		}
		return value;
	}

	static function railsRouteBool(expr:TypedExpr, label:String):Bool {
		return switch (unwrapTypedExpr(expr).expr) {
			case TConst(TBool(value)):
				value;
			case _:
				Context.error('@:railsRoutes $label must be a Bool literal marker value.', expr.pos);
				false;
		}
	}
	...
	function emitRailsTestArtifact(classType:ClassType, funcFields:Array<ClassFuncData>):Void {
		var plan = railsTestArtifacts.prepare(classType, buildContext.railsMode);
		if (plan == null) {
			return;
		}
		var adapter = plan.adapter;
		...
		var lines = RailsTestArtifacts.render(plan, body, railsTestIncludes(funcFields));
		setRailsExtraFile(plan.outputPath, normalizeGeneratedText(lines.join("\n")), classType.pos);
	}

	function emitRailsMailerPreviewArtifact(classType:ClassType, funcFields:Array<ClassFuncData>):Void {
		var plan = railsMailerPreviewArtifacts.prepare(classType, buildContext.railsMode);
		if (plan == null) {
			return;
		}
		...
		for (field in funcFields) {
			...
			RailsMailerPreviewArtifacts.validateMethod(field);
		...
		var lines = RailsMailerPreviewArtifacts.render(plan, body);
		setRailsExtraFile(plan.outputPath, normalizeGeneratedText(lines.join("\n")), classType.pos);
	}
	...
	static function railsTestIncludes(funcFields:Array<ClassFuncData>):Array<String> {
			...
			iterTypedExprRecursive(field.expr, function(expr:TypedExpr) {
				switch (unwrapTypedExpr(expr).expr) {
					case TCall(callee, _):
						var info = staticFieldInfo(callee);
						if (info != null) {
							var moduleName = metaStringParam(info.field.meta, ":railsTestInclude", 0);
							if (moduleName != null) {
								if (!isSafeRubyConstantPath(moduleName)) {
									Context.error("@:railsTestInclude must name a safe Ruby constant path.", info.field.pos);
								}
	...
	function railsTemplateAstBody(classType:ClassType, funcFields:Array<ClassFuncData>, methodName:String):Null<String> {
		for (field in funcFields) {
			...
			var scope = templateScopeFor(classType, field, funcFields);
	...
	static function templateScopeFor(classType:ClassType, field:ClassFuncData, funcFields:Array<ClassFuncData>):RailsTemplateScope {
		...
		return {localNames: localNames, localObjectNames: localObjectNames, viewHelpers: templateViewHelpers(classType, funcFields, field.field.name)};
	}

	static function templateViewHelpers(classType:ClassType, funcFields:Array<ClassFuncData>, renderMethodName:String):Map<String, RailsTemplateHelper> {
		var helpers:Map<String, RailsTemplateHelper> = [];
		var owner = fullTypeName(classType.pack, classType.name);
		for (helper in funcFields) {
			if (!helper.isStatic
				|| helper.field.name == renderMethodName
				|| helper.expr == null
			...
			helpers.set(templateViewHelperKey(owner, helper.field.name), {
				owner: owner,
				field: helper.field,
				args: helper.args,
				body: body,
	...
	static function templateViewHelperKey(owner:String, fieldName:String):String {
	...
	static function cloneTemplateScope(scope:RailsTemplateScope):RailsTemplateScope {
		...
		return {
			...
			localObjectNames: localObjectNames,
			viewHelpers: scope.viewHelpers,
			formBuilderName: scope.formBuilderName
		};
	}
	...
	static function printTemplateExpr(expr:TypedExpr, scope:RailsTemplateScope):String {
		...
		return switch (unwrapped.expr) {
			...
			case TCall(callee = {expr: TField(_, FStatic(classRef, fieldRef))}, params):
				var extensionExpr = rubyExtensionExprCode(callee, params, expr -> printTemplateExpr(expr, scope));
				if (extensionExpr != null) {
					extensionExpr;
				} else {
					var helperExpr = templateViewHelperExpr(classRef.get(), fieldRef.get(), params, scope);
					if (helperExpr != null) {
						helperExpr;
					} else {
						var classType = classRef.get();
						var receiver = rubyNativeName(classType.meta) ?? rubyClassConstantPath(classType);
	...
	static function templateViewHelperExpr(classType:ClassType, field:ClassField, params:Array<TypedExpr>, scope:RailsTemplateScope):Null<String> {
	...
	static function validateTemplateViewHelperBody(helper:RailsTemplateHelper, scope:RailsTemplateScope):Void {
		var allowedLocals:Map<Int, Bool> = [];
		for (arg in helper.args) {
			if (arg.tvar != null) {
				allowedLocals.set(arg.tvar.id, true);
			}
		}
		var invalidLocal:Null<TVar> = null;
		var unsafeCall:Null<{method:String, pos:Position}> = null;
		iterTypedExprRecursive(helper.body, function(expr:TypedExpr) {
			if (invalidLocal != null || unsafeCall != null) {
				return;
			}
			var unwrapped = unwrapTemplateExpr(expr);
			switch (unwrapped.expr) {
				case TLocal(v) if (!allowedLocals.exists(v.id)):
					invalidLocal = v;
				case TCall({expr: TField(_, FStatic(classRef, fieldRef))}, _)
					if (isRailsModelType(classRef.get()) && isUnsafeTemplateViewHelperMethod(fieldRef.get().name)):
					unsafeCall = {method: fieldRef.get().name, pos: unwrapped.pos};
				case TCall({expr: TField(_, access)}, _) if (isUnsafeTemplateViewHelperMethod(fieldAccessRawName(access))):
					unsafeCall = {method: fieldAccessRawName(access), pos: unwrapped.pos};
				case _:
			}
		});
		if (invalidLocal != null) {
			Context.error('View-local helper ${helper.owner}.${helper.field.name} uses local "${invalidLocal.name}". The first RailsHx helper slice can only inline helpers whose return expression depends on helper arguments and constants.',
				helper.field.pos);
		}
		if (unsafeCall != null) {
			Context.error('View-local helper ${helper.owner}.${helper.field.name} calls "${unsafeCall.method}". View-local helpers must stay pure presentation logic; load data or mutate state in controllers/models/services and pass typed locals into the view.',
				unsafeCall.pos);
		}
	}

	static function isRailsModelType(classType:ClassType):Bool {
		return hasMeta(classType.meta, ":railsModel");
	}

	static function isUnsafeTemplateViewHelperMethod(name:String):Bool {
		return [
			"where",
			"whereNot",
			"whereIn",
	...
	function outputRelativeDir(dir:Null<String>, appOwned:Bool):Null<String> {
		...
		if (buildContext.railsMode && appOwned) {
			if (buildContext.usesRailsNativeLayout()) {
				return normalized == "" ? RAILS_GENERATED_SUPPORT_ROOT : RAILS_GENERATED_SUPPORT_ROOT + "/" + normalized;
			}
	...
	function outputRelativePath(path:String, runtimeOwned:Bool):String {
		if (buildContext.railsMode && runtimeOwned) {
			if (buildContext.usesRailsNativeLayout()) {
				return RAILS_VENDOR_RUNTIME_ROOT + "/" + path;
			}
	...
	static function rubyClassConstantPath(classType:ClassType):String {
		if (activeBuildContext.usesRailsNativeLayout() && hasMeta(classType.meta, ":railsCableConnection")) {
			return "ApplicationCable::Connection";
		}
		if (activeBuildContext.usesRailsNativeLayout() && isRailsNativeTopLevelConstant(classType)) {
			return RubyNaming.toConstantName(classType.name);
		}
		return RubyOutputLayout.classConstantOverride(classType.pack, classType.name) ?? rubyConstantPath(classType.pack, classType.name);
	}
	...
	static function hasRubyNoEmit(meta:Null<haxe.macro.Type.MetaAccess>):Bool {
		// `@:rubyNoEmit` is the target-level contract for compile-time-only
		// authoring facades. `@:railsNoEmit` remains as a compatibility alias for
		// early RailsHx sources, but new code should prefer the Ruby-wide name.
		return hasMeta(meta, ":rubyNoEmit") || hasMeta(meta, ":railsNoEmit");
	}
	...
	static function metaIntParam(meta:Null<haxe.macro.Type.MetaAccess>, name:String, index:Int):Null<Int> {
		if (meta == null || meta.extract == null) {
			return null;
		}
		var entries = meta.extract(name);
	...
	static function validateMetadataObjectKeys(fields:Array<RubyMetadataField>, allowed:Array<String>, metadataName:String, pos:Position):Void {
		for (field in fields) {
			if (allowed.indexOf(field.field) == -1) {
				Context.error('$metadataName does not support field "${field.field}".', field.expr.pos);
			}
		}
	}

	static function metadataObjectString(fields:Array<RubyMetadataField>, key:String, pos:Position, metadataName:String = "Target metadata"):String {
		for (field in fields) {
			if (field.field != key) {
				continue;
			}
			return switch (field.expr.expr) {
				case EConst(CString(value, _)): value;
				case _:
					Context.error('$metadataName field "$key" must be a string literal.', field.expr.pos);
					"";
			}
		}
		Context.error('$metadataName is missing required field "$key".', pos);
		return "";
	}

	static function metadataObjectOptionalString(fields:Array<RubyMetadataField>, key:String, pos:Position,
			metadataName:String = "Target metadata"):Null<String> {
		for (field in fields) {
			if (field.field != key) {
				continue;
			}
	...
	static function metadataObjectInt(fields:Array<RubyMetadataField>, key:String, pos:Position, metadataName:String = "Target metadata"):Int {
		for (field in fields) {
			if (field.field != key) {
				continue;
			}
			return switch (field.expr.expr) {
				case EConst(CInt(value, _)): Std.parseInt(value);
				case _:
					Context.error('$metadataName field "$key" must be an integer literal.', field.expr.pos);
					0;
			}
		}
		Context.error('$metadataName is missing required field "$key".', pos);
	...
	static function metadataObjectOptionalInt(fields:Array<RubyMetadataField>, key:String, pos:Position, metadataName:String = "Target metadata"):Null<Int> {
		for (field in fields) {
			if (field.field != key) {
				continue;
			}
	...
	static function newLocalNameScope():LocalNameScope {
		return {names: [], nextByBase: [], capturedSelfAliases: []};
	}
	...
	static function markCapturedSelfAlias(v:TVar):Void {
		...
		localNameScope.capturedSelfAliases.set(v.id, true);
	}

	static function isCapturedSelfAlias(v:TVar):Bool {
		return localNameScope != null && localNameScope.capturedSelfAliases.exists(v.id);
	}

	static function isCapturedSelfAliasDeclaration(v:TVar, init:Null<TypedExpr>):Bool {
		if (init == null || !isCapturedSelfAliasName(v.name)) {
			return false;
		}
		return switch (unwrapTypedExpr(init).expr) {
