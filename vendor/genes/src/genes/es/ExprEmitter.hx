  ...
  public function emitExpr(e: TypedExpr) {
    ...
    switch e.expr {
      ...
      case TFunction(f):
        ...
        this.inValue = 0;
        this.inLoop = false;
        // Check if this is an async function (has __async_marker__ in body)
        var isAsync = false;
        switch (f.expr.expr) {
          case TBlock(exprs) if (exprs.length > 0):
            switch (exprs[0].expr) {
              case TVar(v, {expr: TConst(TBool(true))}) if (v.name == '__async_marker__'):
                isAsync = true;
              case _:
            }
          case _:
        ...
        write(') ');
        // Filter out the __async_marker__ variable when emitting
        if (isAsync) {
          switch (f.expr.expr) {
            case TBlock(exprs):
              ...
              emitExpr({expr: TBlock(filtered), pos: f.expr.pos, t: f.expr.t});
            case _:
      ...
      case TCall({
        expr: TField(_,
          FStatic(_.get() => {module: 'js.Syntax'}, _.get() => {name: 'code'}))
      }, [{expr: TConst(TString(code))}, arg]) if (code == "await {0}"):
        // Special handling for await
        write('await ');
        emitExpr(arg);
      case TCall({
        expr: TField(_,
