...
function expectMigrationSnapshotOperationsOutput() {
  ...
  writeFileSync(join(migrationSnapshotOpsSourceDir, "migrations", "SnapshotOperationsMigration.hx"), [
    ...
    "\t\t\t\tIndex([\"user_id\", \"title\"], {unique: true, includeColumns: [\"amount\"], comment: \"User title lookup\"})",
    "\t\t\t],",
    "\t\t\ttimestamps: true",
    "\t\t}),",
  ...
  for (const expected of [
    ...
    't.index [:user_id, :title], unique: true, include: [:amount], comment: "User title lookup"',
    "t.timestamps",
