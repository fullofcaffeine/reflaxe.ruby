...
function expectMigrationSnapshotOperationsOutput() {
  ...
  writeFileSync(join(migrationSnapshotOpsSourceDir, "migrations", "SnapshotOperationsMigration.hx"), [
    ...
    "\t\t], [",
    "\t\t\tRemoveUniqueConstraint(\"audit_events\", [\"account_id\", \"title\"], {name: \"unique_audit_events_account_title_guarded\"})",
    "\t\t]),",
    "\t\tReversible([",
    ...
    "\t\t], [",
    "\t\t\tRemoveExclusionConstraint(\"audit_events\", \"account_id WITH =\", {name: \"audit_events_account_guarded_exclusion\", usingMethod: \"gist\"})",
    "\t\t]),",
    "\t\tReversible([",
