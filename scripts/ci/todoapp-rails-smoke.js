...
function expectMigrationSnapshotOperationsOutput() {
  ...
  writeFileSync(join(migrationSnapshotOpsSourceDir, "migrations", "SnapshotOperationsMigration.hx"), [
    ...
    "\t\t\tAddColumnIfNotExists(\"audit_events\", \"legacy_notes\", TextColumn({}))",
    "\t\t]),",
    "\t\tAddCompositeIndex(\"audit_events\", [\"account_id\", \"title\"], {lengths: [{column: \"title\", length: 80}], orders: [{column: \"title\", direction: Desc}, {column: \"account_id\", direction: Asc}], includeColumns: [\"amount\"], comment: \"Account title lookup\"}),",
    "\t\tReversible([",
  ...
  for (const expected of [
    ...
    'add_index :audit_events, [:account_id, :title], length: {title: 80}, order: {title: :desc, account_id: :asc}, include: [:amount], comment: "Account title lookup"',
