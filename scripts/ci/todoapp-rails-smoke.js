...
function expectMigrationSnapshotOperationsOutput() {
  ...
  writeFileSync(join(migrationSnapshotOpsSourceDir, "migrations", "SnapshotOperationsMigration.hx"), [
    ...
    "\t\t\t\tIndex([\"user_id\", \"title\"], {unique: true, includeColumns: [\"amount\"], nullsNotDistinct: true, comment: \"User title lookup\"})",
    "\t\t\t],",
    "\t\t\ttimestamps: true",
    "\t\t}),",
    ...
    "\t\t\t\tIndex([\"bulk_status\"], {name: \"index_audit_events_on_bulk_status\", includeColumns: [\"metadata\"]})",
    "\t\t\t],",
    "\t\t\tforeignKeys: [",
    "\t\t\t\t{toTable: \"users\", options: {column: \"reviewer_id\", name: \"fk_audit_events_reviewers\", validate: false}}",
    "\t\t\t],",
    "\t\t\tremoveIndexes: [",
    ...
    "\t\t\tAddColumnIfNotExists(\"audit_events\", \"legacy_notes\", TextColumn({}))",
    "\t\t]),",
    "\t\tAddCompositeIndex(\"audit_events\", [\"account_id\", \"title\"], {includeColumns: [\"amount\"], comment: \"Account title lookup\"}),",
    "\t\tReversible([",
  ...
  for (const expected of [
    ...
    't.index [:user_id, :title], unique: true, include: [:amount], nulls_not_distinct: true, comment: "User title lookup"',
    "t.timestamps",
    ...
    'add_index :audit_events, [:account_id, :title], include: [:amount], comment: "Account title lookup"',
