...
function expectMigrationSnapshotOperationsOutput() {
  ...
  writeFileSync(join(migrationSnapshotOpsSourceDir, "migrations", "SnapshotOperationsMigration.hx"), [
    ...
    "\t\tReversible([",
    "\t\t\tChangeTable(\"audit_events\", {",
    "\t\t\t\tremoveReferences: [{name: \"reviewer\", options: {indexName: \"index_audit_events_on_reviewer_id\"}, ifExists: true}]",
    "\t\t\t})",
    "\t\t], [",
    "\t\t\tChangeTable(\"audit_events\", {",
  ...
  for (const expected of [
    ...
    't.remove_references :reviewer, index: { name: "index_audit_events_on_reviewer_id" }, if_exists: true',
