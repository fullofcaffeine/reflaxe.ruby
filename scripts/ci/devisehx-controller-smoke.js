...
function writePositiveSources(dir) {
  ...
  mkdirSync(join(dir, "fakecompanion"), { recursive: true });
  mkdirSync(join(dir, "models"), { recursive: true });

  writeFileSync(join(dir, "fakecompanion", "Audit.hx"), [
    "package fakecompanion;",
    "",
    "import rails.action_controller.Base;",
    "",
    "// Minimal non-Devise companion proving the compiler contract is generic.",
    "@:rubyNoEmit",
    "@:rubyAllowRaw",
    "class Audit {",
    "\t@:railsFilterMethod(\"verify_actor!\")",
    "\tpublic static final verify:Bool = false;",
    "",
    "\t@:railsRequiresFilter(\"verify_actor!\")",
    "\t@:rubyExtensionExpr({schema: 1, template: \"current_actor()\", valueStart: 1})",
    "\tpublic static function actor(controller:Base):String {",
    "\t\treturn \"\";",
    "\t}",
    "}",
    "",
  ...
  writeFileSync(join(dir, "app", "auth", "AdminAuth.hx"), [
    ...
    "",
    "\t@:railsFilterMethod(\"authenticate_admin!\")",
    "\tpublic static final authenticate:AuthFilter<Admin> = Auth.require(scope);",
    "",
    "\t@:railsRequiresFilter(\"authenticate_admin!\")",
  ...
  writeFileSync(join(dir, "controllers", "DashboardController.hx"), [
    ...
    "\t\tbeforeAction(Audit.verify, {only: [index]});",
    "\t}",
    "",
...
function haxeArgs(src, out, options = {}) {
  ...
  if (options.strictCurrentRequired) {
    args.splice(6, 0, "-D", "railshx_strict_required_filters");
  }
  return args;
}
