...
const runtimeDir = join(root, "test", ".generated", "scaffold_cli_runtime");
...
assertIncludesFrom(rspecScaffoldDir, "test_haxe/models/CommentHaxeSpec.hx", [
  '@:railsTestAdapter("rails.rspec")',
  '@:railsTest("models/comment_haxe_spec")',
  "class CommentHaxeSpec extends ModelTestCase",
  "notNil(Comment.all());",
]);
assertIncludesFrom(rspecScaffoldDir, "src_haxe/Main.hx", [
  "import test_haxe.models.CommentHaxeSpec;",
  "var modelTest:Class<CommentHaxeSpec> = CommentHaxeSpec;",
]);
if (!compileWithFirstAvailableReflaxe(join(rspecScaffoldDir, "ruby"), { projectDir: rspecScaffoldDir })) {
  console.error("Unable to compile RSpec scaffolded Haxe project through Reflaxe.");
  process.exit(1);
}
assertIncludesFrom(rspecScaffoldDir, "ruby/spec/generated/models/comment_haxe_spec.rb", [
  "# Generated by RailsHx from @:railsTest.",
  'RSpec.describe "CommentHaxeSpec", type: :model do',
  'it "generated model relation is typed" do',
  "expect(Comment.all()).not_to be_nil",
]);
...
assertIncludes("ruby/app/controllers/todos_controller.rb", [
  "todos = Todo.all().to_a()",
  'self.render(template: "controllers/todos/index", locals: {title: "TodosController#index", todos: todos})',
...
assertIncludes("ruby/test/generated/models/todo_haxe_test.rb", [
  ...
  "assert_not_nil(Todo.all())",
]);

// This is a seam smoke, not a broad Rails behavior test: it verifies that
// Rails can boot the scaffold artifacts, migrate them, render generated HHX
// output, and run the generated Rails/Minitest files.
materializeRuntimeScaffold();
syntaxCheckRuntime([
...
if (railsProbe.status !== 0) {
  if (requireRails) {
    process.stdout.write("[scaffold-cli] Rails gems missing; running bundle install because REQUIRE_RAILS=1.\n");
    run("bundle", ["install"], { cwd: runtimeDir });
  } else {
    process.stdout.write("[scaffold-cli] Skipped scaffold Rails runtime test; Rails gems are unavailable.\n");
    process.stdout.write("[scaffold-cli] Set REQUIRE_RAILS=1 to make this lane mandatory.\n");
  }
}
if (railsProbe.status === 0 || requireRails) {
  run("bundle", ["exec", "ruby", "bin/rails", "db:migrate"], {
    cwd: runtimeDir,
    env: { ...process.env, RAILS_ENV: "test" },
  });
  run("bundle", ["exec", "ruby", "bin/rails", "test"], {
    cwd: runtimeDir,
    env: { ...process.env, RAILS_ENV: "test" },
  });
}
...
function assertIncludes(relativeFile, expectedLines) {
  assertIncludesFrom(outputDir, relativeFile, expectedLines);
}

function assertIncludesFrom(baseDir, relativeFile, expectedLines) {
  const content = readFileSync(join(baseDir, relativeFile), "utf8");
...
function materializeRuntimeScaffold() {
  rmSync(runtimeDir, { force: true, recursive: true });
  copyTree(join(outputDir, "ruby", "app"), join(runtimeDir, "app"));
  copyTree(join(outputDir, "ruby", "config"), join(runtimeDir, "config"));
  copyTree(join(outputDir, "ruby", "db", "migrate"), join(runtimeDir, "db", "migrate"));
  copyTree(join(outputDir, "ruby", "test"), join(runtimeDir, "test"));
...
`);
  ...
  writeRuntimeFile("Rakefile", `require_relative "config/application"

Rails.application.load_tasks
`);

  writeRuntimeFile("config/application.rb", `require "rails"
require "active_record/railtie"
require "action_controller/railtie"
require "action_view/railtie"

module HXRubyScaffoldRuntime
...
`);
  ...
  writeRuntimeFile("config/environment.rb", `require_relative "application"

Rails.application.initialize!
`);

  writeRuntimeFile("bin/rails", `#!/usr/bin/env ruby
APP_PATH = File.expand_path("../config/application", __dir__)
require_relative "../config/boot"
require "rails/commands"
`);

  writeRuntimeFile("config/database.yml", `test:
  adapter: sqlite3
  database: db/test.sqlite3
`);

  writeRuntimeFile("app/models/application_record.rb", `class ApplicationRecord < ActiveRecord::Base
  self.abstract_class = true
end
`);

  writeRuntimeFile("app/controllers/application_controller.rb", `class ApplicationController < ActionController::Base
end
`);

  writeRuntimeFile("test/test_helper.rb", `ENV["RAILS_ENV"] ||= "test"
...
`);
  ...
  writeRuntimeFile("test/controllers/todos_controller_test.rb", `require "test_helper"

class TodosControllerTest < ActionDispatch::IntegrationTest
  test "renders generated HHX index" do
    Todo.create!(title: "Typed scaffold", is_completed: false)

    get "/todos"

    assert_response :success
    assert_includes @response.body, "Typed scaffold"
  end

  test "creates through generated strong params" do
    assert_difference("Todo.count", 1) do
      post "/todos", params: {todo: {title: "Runtime create", is_completed: "0"}}
    end
...
function syntaxCheckRuntime(relativeFiles) {
  for (const relativeFile of relativeFiles) {
    run("ruby", ["-c", join(runtimeDir, relativeFile)]);
  }
}
...
function writeRuntimeFile(relativePath, content) {
  const fullPath = join(runtimeDir, relativePath);
...
function compileWithFirstAvailableReflaxe(rubyOutput, options = {}) {
  ...
  for (const reflaxeSrc of reflaxeCandidates) {
    ...
    const result = run("haxe", [
      ...
      join(root, "src"),
      "-cp",
      join(projectDir, "src_haxe"),
      ...(existsSync(join(projectDir, "test_haxe")) ? ["-cp", projectDir] : []),
      "-cp",
      reflaxeSrc,
      "--macro",
