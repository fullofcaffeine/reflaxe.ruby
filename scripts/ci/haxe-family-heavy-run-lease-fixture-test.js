...
function acquire(leasePath, identityMap, overrides = {}) {
  return acquireLease({
    ...
    repository: overrides.repository || "reflaxe-ruby-fixture",
...
function wrapperArgs(leasePath, command, overrides = {}) {
  return [
    ...
    leasePath,
    "--label",
    overrides.label || "reflaxe-ruby-fixture",
    "--",
    ...command,
  ];
}
...
async function main() {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "reflaxe-ruby-heavy-lease-"));
