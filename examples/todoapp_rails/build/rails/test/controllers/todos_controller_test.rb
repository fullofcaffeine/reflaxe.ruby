...
class TodosControllerTest < ActionDispatch::IntegrationTest
  ...
  test "users page renders admin-only typed CRUD" do
    ...
    assert_redirected_to "/users"
    follow_redirect!
    assert_includes @response.body, "User updated"
    managed.reload
    ...
    assert_redirected_to "/users"
    follow_redirect!
    assert_includes @response.body, "User removed"
  end

  test "users create re-renders validation feedback for duplicate emails and password mismatches" do
    user = create_user!(name: "owner", email: "owner-user-errors@example.test", role: "admin")
    ...
    assert_no_difference "User.count" do
      post "/users", params: {
        user: {
          name: "Dupe",
          email: "member-user-errors@example.test",
          role: "member",
    ...
    assert_response :unprocessable_entity
    assert_includes @response.body, "app-flash"
    assert_includes @response.body, "Could not save user"
    assert_includes @response.body, "Review user details"
    assert_match(/Email has already been taken|Password confirmation doesn&#39;t match|Password confirmation doesn&#x27;t match/, @response.body)
    assert_includes @response.body, "member-user-errors@example.test"
  end
