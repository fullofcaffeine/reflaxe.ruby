...
RailsHx has the first typed ActionView seam: controllers render through `ViewMacro.renderTemplate(this, (Template.named("...") : Template<TLocals>), locals)`, which type-checks locals in Haxe and lowers to a normal Rails `render(template:, locals:)` call. `@:railsTemplate(...)` classes now materialize Rails-native ERB artifacts into generated output from Haxe-owned template bodies. Raw ERB requires explicit `@:railsAllowRawErb`; the destination is a fuller typed Rails template layer inspired by `../haxe.elixir.codex`'s HXX/HEEx architecture.
