...
- `views/UserSwitcherView.hx` is the typed first-party Rails session/user panel, not a Devise replacement: `User.f.id` drives the hidden form field, route helper externs drive sign-in/sign-out URLs, `shared/TodoHooks` marks the Turbo-enhanced forms, and typed `<turbo_frame>` gives the "Manage users" link a standard Hotwire frame target. It also demonstrates both Rails-native HHX helper shapes: simple `<button_to>Clear session</button_to>` lowers to `button_to "Clear session", sign_out_path, method: :delete`, while nested `<button_to><span>Open in frame</span></button_to>` lowers to Rails' block form `button_to users_path do ... end`.
