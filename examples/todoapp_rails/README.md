...
- `controllers/ChatMessagesController.hx` demonstrates a Turbo-style mutation seam: typed strong params create a chat message, Turbo Stream requests replace a typed HHX panel, and HTML fallback redirects through normal Rails.
...
- `client/TodoClient.hx` compiles through Genes to `app/javascript/railshx/todo_client.js` plus importmap-friendly ES modules under `app/javascript/railshx/**`. It owns progressive enhancement: typed Turbo lifecycle hooks, smooth same-page navigation, scroll-position preservation after create, chat/session form binding, and transient typed-status flashes. The flash timers use `@:async` Haxe methods and `await(Async.delay(...))`, which emit native JavaScript `async`/`await` while keeping completion typed in Haxe. Todo, chat, and session mutations return typed Turbo Stream updates for Turbo requests while retaining normal Rails redirects for HTML fallback.
