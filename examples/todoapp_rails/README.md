...
- Haxe-authored typed ActionView partials through `@:railsTemplateAst(...)`, Rails HHX inline markup, `H`, `HtmlNode`, and `HtmlAttr`; the compiler type-checks embedded expressions such as `todo.title`, typed partial locals, route helper calls, and typed form locals before emitting ERB.
...
- `views/TodoFormView.hx` uses HHX inline markup (`<form_with>`, `<hidden_field>`, `<text_field>`, `<submit>`) for a typed Rails form partial and generates `app/views/controllers/todos/_typed_form.html.erb`.
...
RailsHx has the first typed ActionView seams: controllers render through `ViewMacro.renderTemplate(this, (Template.named("...") : Template<TLocals>), locals)`, which type-checks locals in Haxe and lowers to a normal Rails `render(template:, locals:)` call. `@:railsTemplate(...)` classes materialize Rails-native ERB artifacts into generated output from Haxe-owned template bodies. `@:railsTemplateAst("render")` adds the first HXX-style typed template path: a static method returns `HtmlNode`, preferably authored as Rails HHX inline markup such as `return <div>${todo.title}</div>`. `RailsInlineMarkup` rewrites that markup into the same typed `HtmlNode`/`HtmlAttr` AST that `H` can build manually, Haxe validates embedded expressions/loop binders/partial locals/route helpers/form locals, and the compiler emits ERB. Raw ERB requires explicit `@:railsAllowRawErb`; the destination is a fuller typed Rails template layer inspired by `../haxe.elixir.codex`'s HXX/HEEx architecture.
