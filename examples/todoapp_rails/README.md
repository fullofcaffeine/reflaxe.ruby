...
- `src/shared/ChatRoomHooks.hx` and `src/shared/ChatRoomContract.hx` split the chat room contract into browser-safe hooks and a server-side `@:hotwireContract` declaration. The macro generates typed stream, target, and row-template accessors from one `Template<ChatMessageLocals>` source while the explicit locals mapper and Playwright readiness selectors remain reviewable.
