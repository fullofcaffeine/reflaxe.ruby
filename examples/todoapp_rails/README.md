...
- Haxe-authored typed ActionView partials through `@:railsTemplateAst(...)`, `H`, `HtmlNode`, and `HtmlAttr`; the compiler type-checks embedded expressions such as `todo.title` and typed partial locals before emitting ERB.
...
- `views/TodoDashboardView.hx` composes the summary partial through typed `H.partial(...)` locals and generates `app/views/controllers/todos/_dashboard.html.erb`.
