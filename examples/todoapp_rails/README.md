...
- `src/views/ChatPanelView.hx` is the typed chatroom panel: `<turbo_stream_from>` subscribes through `ChatRoomContract.messageStream()`, `ChatMessage.f.body` drives the form field, `Routes.chatMessagesPath()` drives submission, and the initial message loop composes the typed row partial through the shared chat room contract. Like todos, chat authorship comes from `current_user` in the controller instead of a spoofable hidden field.
...
- `src/shared/ChatRoomHooks.hx` and `src/shared/ChatRoomContract.hx` split the chat room contract into browser-safe hooks and server-only Turbo/ActionView types, keeping stream names, targets, templates, locals, and Playwright readiness selectors on one Haxe-owned surface.
