...
- `views/UserSwitcherView.hx` is the typed session/user panel: `User.f.id` drives the hidden form field, route helper externs drive sign-in/sign-out URLs, `shared/TodoHooks` marks the Turbo-enhanced forms, and typed `<turbo_frame>` gives the "Manage users" link a standard Hotwire frame target.
