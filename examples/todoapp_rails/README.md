...
- `controllers/TodosController.hx` is a RailsHx controller using typed params, typed Devise current-user lookup, typed relation queries, and route helpers. The board is public so guests can reach the auth panel; mutations stay protected through `beforeAction(UserAuth.authenticate, {except: [index]})`.
...
- `views/UserSwitcherView.hx` is the typed DeviseHx auth/user panel: Devise route helper externs drive `newUserSessionPath()` and `destroyUserSessionPath()`, the Haxe-owned guest route drives `guestSignInPath()`, `shared/TodoHooks` marks the Turbo-enhanced forms, and typed `<turbo_frame>` gives the "Manage users" link a standard Hotwire frame target. It demonstrates both link-style and button-style Rails HHX helper shapes while Devise owns session persistence.
- `views/ChatPanelView.hx` is the typed chatroom panel: `<turbo_stream_from>` subscribes to the typed stream, `ChatMessage.f.body` and `ChatMessage.f.userId` drive form fields, `Routes.chatMessagesPath()` drives submission, and the initial message loop composes the typed row partial through `Template.of(ChatMessageView)`.
...
- `views/UserManagementView.hx` is a second HHX page showing typed user fields, role helpers, route-helper links, and the matching `<turbo_frame>` response that Turbo extracts into the todo board while preserving direct `/users` fallback navigation.
