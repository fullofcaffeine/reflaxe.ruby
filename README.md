...
The target is still pre-1.0, but the current baseline supports executable Ruby smoke fixtures, shared `hxruby` runtime files, Ruby interop metadata, and a Rails MVP with typed ActiveRecord models, ActionController helpers, params macros, routes generation, and scaffold tooling.
...
The canonical Rails example is `examples/todoapp_rails`.
