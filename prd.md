...
locked to Rails `8.1.3.1`. Generated fixtures retain a Rails `>= 7.0` floor without
...
Rails: Rails `8.1` is the supported stable line, exercised at Rails `8.1.3.1`; other Rails versions may work but are unverified rather than rejected.
...
The current RailsHx stable runtime lane uses Rails `8.1`, locked at `8.1.3.1`, on
